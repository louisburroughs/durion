...
| Putaway rules | `stock.putaway.rule` (product/category/package-type → destination, sublocation strategies none/last-used/closest), rule specificity resolution | `PutawayRule` (priority + criteria JSON → destination), `PutawayGenerationServiceImpl` suggestion + fallback reasons, task claim with pessimistic lock | Comparable core. Odoo adds sublocation strategies and category/package-type matching |
| Storage capacity | `stock.storage.category`: max weight, per-product/package-type qty capacity, `allow_new_product` (empty/same/mixed), checked against current+forecast | `ext_storage_location.maxUnitCapacity` + `PutawayValidationServiceImpl` (SKU compatibility, capacity + tolerance override, source on-hand check; override permissions) | Durion has real enforcement with an override/audit model Odoo lacks. Odoo is richer per-product/weight. Mostly parity for POS needs |
