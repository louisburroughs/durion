...
**OpenAPI Source:** `pos-invoice/openapi.json`
...
This domain exposes **2** REST API endpoints:
...
- `partyId` (path, Required, string): Party/Customer ID
...
- `partyId` (path, Required, string): Party/Customer ID
- `X-User-Id` (header, Optional, string): User ID performing the operation
...
Billing rules configuration for a commercial account (CAP:092)

**Fields:**

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `createdAt` | string (date-time) | No | Timestamp when the rules were created |
| `id` | string (uuid) | No | Unique identifier for the billing rules |
| `invoiceDeliveryMethod` | string | Yes | How invoices should be delivered to the customer |
| `invoiceGroupingStrategy` | string | Yes | How to group charges into invoices |
| `partyId` | string | Yes | Party/Customer ID these rules apply to |
| `paymentTermsCode` | string | Yes | Payment terms code (e.g., NET30, NET60, DUE_ON_RECEIPT) |
| `purchaseOrderRequired` | boolean | No | Whether a purchase order is required for invoicing |
| `updatedAt` | string (date-time) | No | Timestamp when the rules were last updated |
| `updatedBy` | string | No | User ID who last updated the rules |
...
- OpenAPI Specification: `pos-invoice/openapi.json`
...
- Cross-Domain Integration: `domains/billing/.business-rules/CROSS_DOMAIN_INTEGRATION_CONTRACTS.md`
- Error Codes: `domains/billing/.business-rules/ERROR_CODES.md`
