...
| **inventory.scrap.posted** | pos-inventory (`ScrapPostedV1`, `inventory.events.v1`) | Scrap document posted (`SCRAP_OUT`) | {scrapId, sku, locationId, storageLocationId, quantity, reasonCode, unitCost, costSource, workorderId, occurredAt} | Dr Inventory Shrinkage (5100) / Cr Inventory (1300) for quantity × unitCost; uncosted (`costSource=NONE`) recorded and skipped | ✅ Implemented (#1043, `InventoryShrinkagePostingService`) |
| **inventory.adjustment.posted** | pos-inventory (`InventoryAdjustedV1`, `inventory.events.v1`) | Approved cycle-count or manual adjustment posted (`COUNT_VARIANCE_IN/OUT`, `ADJUSTMENT_IN/OUT`) | {adjustmentId, adjustmentKind, ledgerEventType, ledgerEntryId, sku, locationId, taskId, reasonCode, quantityDelta (signed), unitCost, costSource, occurredAt} | Loss (delta < 0): Dr `ADJUSTMENT_LOSS` (5100) / Cr `INVENTORY_ASSET` (1300); gain (delta > 0): Dr 1300 / Cr `ADJUSTMENT_GAIN` (5100, owner decision D2 final); posting category `INVENTORY_ADJUSTMENT`; uncosted recorded and skipped | ✅ Implemented (producer durion-positivity-backend#2190, consumer #2191, `InventoryAdjustmentPostingService`; [SPEC-inventory-adjustment-gl-posting.md](../SPEC-inventory-adjustment-gl-posting.md)) |
| **inventory.product-value.changed** | pos-inventory (`ProductValueChangedV1`, `inventory.events.v1`) | Manual cost revaluation applied (odoo-parity J4) | {revaluationId, sku, costingMethod, previousUnitCost, newUnitCost, onHandQuantity, totalValueDelta, reason, actor, occurredAt} | write-up (`totalValueDelta > 0`): Dr Inventory (1300) / Cr `REVALUATION_OFFSET` (5000 COGS, owner decision D7 final); write-down: Dr `REVALUATION_OFFSET` / Cr Inventory; posting category `INVENTORY_REVALUATION`; zero delta posts no entry, recorded `PROCESSED` | ✅ Implemented (durion-positivity-backend#2193, `InventoryRevaluationPostingService`) |
