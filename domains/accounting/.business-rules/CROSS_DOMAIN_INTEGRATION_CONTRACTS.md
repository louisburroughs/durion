...
| **inventory.adjustment.posted** | pos-inventory (`InventoryAdjustedV1`, `inventory.events.v1`) | Approved cycle-count or manual adjustment posted (`COUNT_VARIANCE_IN/OUT`, `ADJUSTMENT_IN/OUT`) | {adjustmentId, adjustmentKind, ledgerEventType, ledgerEntryId, sku, locationId, taskId, reasonCode, quantityDelta (signed), unitCost, costSource, occurredAt} | Loss (delta < 0): Dr `ADJUSTMENT_LOSS` (5100) / Cr `INVENTORY_ASSET` (1300); gain (delta > 0): Dr 1300 / Cr `ADJUSTMENT_GAIN` (account: owner decision D2); posting category `INVENTORY_ADJUSTMENT`; uncosted recorded and skipped | 📐 Specified — [SPEC-inventory-adjustment-gl-posting.md](../SPEC-inventory-adjustment-gl-posting.md) (durion-positivity-backend#2186); producer and consumer not yet built |
