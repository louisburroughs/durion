...
- `vendorId` (query, Required, string): Vendor UUID
...
- `organizationId` (query, Required, string): Organization identifier
- `page` (query, Optional, integer): Page index (0-based)
- `size` (query, Optional, integer): Page size
...
- `candidateId` (path, Required, string):
...
- `invoiceEventId` (path, Required, string):
...
| `finalPostingReferenceId` | string | No |  |
| `journalEntryId` | string (uuid) | No |  |
| `mappingVersionAttempted` | string | No |  |
| `organizationId` | string (uuid) | No |  |
| `payload` | object | No |  |
| `processedAt` | string (date-time) | No |  |
| `receivedAt` | string (date-time) | No |  |
| `resolvedByUserId` | string | No |  |
| `sequenceNumber` | integer (int64) | No |  |
| `sourceSystem` | string | No |  |
| `status` | string | No |  |
| `transactionDate` | string (date-time) | No |  |
...
| `invoiceBalanceAfter` | number | No |  |
| `invoiceBalanceBefore` | number | No |  |
| `invoiceId` | string (uuid) | No |  |
| `invoiceStatus` | string | No |  |
| `paymentApplicationId` | string (uuid) | No |  |
