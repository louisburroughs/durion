...
`domains/general/mcp-server/operations/dashboards/nlti-overview.md`, which documents the two provisioned dashboards
...
1. Name: HighNLTIErrorRate
   ...
   - Severity: P1
   - Runbook: domains/general/mcp-server/operations/runbooks/authz-outage.md
...
2. Name: HighNLTIRequestLatency
   - Trigger: Loki — `quantile_over_time(0.99, TELEMETRY | unwrap latency_totalMs [5m]) > 2000`
     ...
     nothing samples.
   - Severity: P2
   - Runbook: domains/general/mcp-server/operations/runbooks/downstream-timeout.md
...
3. Name: AuditWriteFailuresDetected
   ...
   - Severity: P1
   - Runbook: domains/general/mcp-server/operations/runbooks/audit-storage-failure.md
...
4. Name: NLTIPlanningOrExecutionLatencyAnomaly
   ...
   - Severity: P2
   - Runbook: domains/general/mcp-server/operations/runbooks/planning-failure.md
...
- The Prometheus half is **not** materialized (checked 2026-09-16): `observability/prometheus.yml` loads no
  `rule_files` and its `alertmanagers` target list is empty, so rules 1, 3 and 4, and the Prometheus parts of 9 and 11,
  exist only in this document.
...
- Rules 7, 9, 10 and 11 name both the signal available today and the telemetry field that will
  ...
  `domains/general/mcp-server/operations/dashboards/nlti-overview.md`), switch the rule and delete the substitute.
