---
type: Plan
title: Analytics Capability Plan — Waves 1–3
domain: general
status: active
description: Three-wave plan to make twenty analytics questions answerable through pos-mcp-server; Wave 1 delivered, Wave 2 built with its exit gate unpassed, Wave 3 proposed.
tags: [mcp-server, general]
---

# Analytics Capability Plan — Waves 1–3

Status: Wave 1 delivered; Wave 2 built (#1596–#1601 closed) but its exit gate is not recorded as passed — the last
recorded chat-path run scored 5/12 (2026-09-03, [window-shape evidence](archive/gate-runs/wave-2/2026-09-03-window-shape-evidence.md));
Wave 3 proposed. Reconciled 2026-09-16 when this plan moved to durion.
Date: 2026-08-30
...
Related: [Tool selection architecture](tool-selection-architecture.md), [MCP server architecture](architecture.md).
The permission-based tool-selection spec this plan originally cited was removed in the Spring AI migration; its implemented behavior is in the architecture document.
...
- Each gate run is recorded: question id, pass/fail per criterion, tool trace, token cost, **and
  ...
  from a reworded question. Results table checked in under [`archive/gate-runs/`](archive/gate-runs/) per wave (Wave 1–2 records are historical).
...
- Descriptions must spell out the row shape (`customerId/vendorId, name, current, days31To60,
  ...
  **Resolved (2026-09-01, issue #1604) — A/R aging basis.** The defect recorded above (A/R ignoring
  ...
  `domains/general/mcp-server/archive/gate-runs/2026-09-01-ar-aging-basis-change.md`.
...
([ADR-0057](../../../docs/adr/0057-analytics-money-measure-semantics-and-ownership.adr.md)), which every later
...
opens with one `resolveDateWindow` call (`DateWindowFacadeTool`, `domains/general/mcp-server/archive/gate-closeout-plan-1660-1676.md`
