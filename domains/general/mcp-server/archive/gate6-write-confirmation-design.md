---
type: Design
title: Gate 6 — Write-Action Confirmation Gate (design)
domain: general
status: historical
...
5. Execute the **exact persisted args** (`args_json`) — **never re-parse the user's text** (the
   documented gate-mismatch failure mode; see `domains/general/mcp-server/operations/runbooks/confirmation-gate-mismatch.md`).
