---
type: Design
title: Natural-Language Interface Design — pos-mcp-server
domain: general
status: historical
...
4. User confirms → emit `CONFIRMATION` → execute the **exact persisted args** (never re-parse on confirm — re-parsing is the documented gate-mismatch failure mode; see `domains/general/mcp-server/operations/runbooks/confirmation-gate-mismatch.md`).
