---
type: Audit
title: RBAC Permission / Role Audit — August 2026
description: Governs how a platform permission is retired — bit indexes are permanent and never reused, grants retire by versioned migration, definitions never do — records the ACCOUNT_MANAGER→AR / CONTROLLER→accounting-management role split, holds the triage backlog of codes granted or required but not yet enforced, and keeps open the unidentified actor behind the out-of-band SYSTEM_ADMINISTRATOR grants on alpha.
status: current
---
...
## Current counts are derived, not quoted here

**Every headline number below is an as-of measurement of audited commit `09d7eef`, taken
before any of §7's tasks executed. Do not quote them as current.** Permission counts move
on every grant, gate and migration, so the only trustworthy figure is the one the script
prints now. From the `durion-positivity-backend` repo root:

```bash
python3 scripts/audit-rbac.py --check   # gated drift vs. scripts/rbac-audit-baseline.json
python3 scripts/audit-rbac.py audit.json   # full report; writes the machine-readable audit
```

`--check` last read green on 2026-09-20: no new authorization drift, **0 unreachable
operations**, and 13 accepted exceptions across 4 categories — 1 `required_ungranted`
(`people:timeEntry:`, the parser artifact), 11 `granted_unrequired` (§7 task 5's deferred
codes), 1 `required_no_bit` (the same artifact), 0 `granted_no_bit`. The exceptions and
their justifications are the reviewable record in `scripts/rbac-audit-baseline.json`; that
file, not this document, is the current backlog.

Reproduce the underlying cross-reference with `python3 scripts/audit-rbac.py audit.json`
from the `durion-positivity-backend` repo root — no build, no database, runs in seconds.
The script cross-references five sources of truth:
...
"Required" below means the union of B and C (381 codes). Headline disagreements **as
measured at `09d7eef`, before execution — historical, superseded by `--check` above**:
...
*As measured at `09d7eef`, before execution.* 112 permission codes were required by at
least one operation and granted to **no role in the seed — ADMIN included**; 229 contract
operations had no granted alternate at all. Both are now zero-or-artifact: `--check`
reports 0 unreachable operations and one required-but-ungranted entry, the
`people:timeEntry:` parser artifact. The findings below are kept for the defect shapes
they name, not for their counts.
