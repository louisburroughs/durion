---
type: Spike
title: Location Scope and Effective Dating in the JWT — Spike Findings
description: Establishes that location scope is enforced nowhere today, that pos-people's effective-dated EmployeeLocationAssignment (not pos-security-service's unused role_assignments.scope_type) owns user→location scope, and that scope can ride the JWT as additive loc_fin_bits/loc_oth_bits/loc_scope claims without touching the permission bitset or CATALOG_VERSION — the evidence base ADR-0061 decides from.
status: reference
---
...
#1499/#1512 (RBAC audit), `durion/domains/security/rbac-permission-role-audit-2026-08.md`
...
**Result as of 2026-09-07: 77 endpoints, 15 modules, 0 with any location-ownership check.**

> **The count drifts; the finding does not.** 77/15 is a measurement of the tree on
> 2026-09-07, not a constant. The same parser now runs in CI as `scripts/audit-rbac.py`
> section F (§7), and on 2026-09-20 `python3 scripts/audit-rbac.py --check` from the
> `durion-positivity-backend` repo root reported **91 location-scope decisions recorded
> across 14 modules and 96 endpoints checked against their `@PreAuthorize`**. Quote the
> command's output, not this table. The "0 with any location-ownership check" half is also
> now historical: ADR-0061 was accepted and #1871/#1872 began rolling enforcement out, so
> the CI summary today classifies each operation as `gate` / `narrow` / `unscoped` and
> records the decision per operation in `<module>/location-scope.yaml`. What this section
> still establishes is the finding that mattered — the demand was platform-wide, not the
> "three endpoints" #1375 hypothesised.
