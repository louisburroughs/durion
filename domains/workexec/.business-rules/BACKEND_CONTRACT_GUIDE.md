...
| Dispatch a workorder to a position | `assignServicePosition` | PUT | `/v1/workorders/{workorderId}/position` | Body `{resourceType: BAY\|MOBILE_UNIT\|HOLD, resourceId?, reason?}`; for HOLD `resourceId` defaults to the workorder's own `locationId` and any other value is a 422. A BAY or MOBILE_UNIT that is not active is 422 `SERVICE_POSITION_INACTIVE`. Auth `workorder:operationalContext:override`. See the rules note below. (durion-positivity-backend#1983, #1984, #2001) |
...
- A `BAY` or `MOBILE_UNIT` must be **active** (durion-positivity-backend#2001): a bay out of service or a mobile unit that is not deployed is 422 `SERVICE_POSITION_INACTIVE`, and the workorder keeps the position it had. Its own code rather than `SERVICE_POSITION_INVALID` because the position is real and at the right site — the UI can offer "bring the bay back into service" rather than "that is not a valid position". A position the backend has not replicated yet is still 422 `SERVICE_POSITION_INVALID`. A position that goes inactive while a workorder is already on it releases nothing: the job stays put and the dispatch board flags it.
- **`ASSIGNED` means a technician *and* somewhere to work** (durion-positivity-backend#2010, #2011): a current technician **and** a `BAY` or `MOBILE_UNIT`. A `HOLD` is a parking space, not a place work happens, so it does not count, and either half missing is `APPROVED`. So assigning a technician to an unplaced workorder leaves it `APPROVED` and placing it then makes it `ASSIGNED` — either order, same answer — while releasing the only technician, or the position, reverts it to `APPROVED`. A hand-over (reassigning the technician, moving bay to bay) keeps it `ASSIGNED`. The UI should read the status back rather than predicting it, and must not show "assigned" for a workorder nobody holds.
