...
- **The dispatch board names one holder and one plan** (durion-positivity-backend#2058). On `WorkorderSummary`, `assignedMechanicId` is the **technician of record** — the workorder's current `technician_assignment` row — and is `null` when the aggregate holds nobody. It never falls back to the legacy `mechanic_ids` column, which it used to: a workorder whose only mechanic arrived through the appointment or override path showed as held, and `reassignTechnician` then answered 409 `TECHNICIAN_NOT_ASSIGNED` for a row the board had just labelled "assigned to M". Key assign-vs-reassign off this field; a non-null value is exactly "a current technician exists", which is one half of `ASSIGNED` and not `ASSIGNED` itself, so an `APPROVED` workorder may legitimately carry one (technician, no bay yet).
- `plannedMechanicIds[]` on the same summary is that legacy column (#1658), stated honestly: who scheduling or a dispatch override expects to work the job. It is **scheduling intention, never custody** (DECISION-INVENTORY-021, DECISION-INVENTORY-022) — unvalidated against the person and staffing replicas, not a crew model, and no authority for any technician endpoint. It is published because a dispatcher should see who a job is penciled in for, and because a planned mechanic who is on PTO, at the wrong site or missing a required certification still raises a dashboard conflict. The `mechanicIds` array on `workorder.events.v1`'s `WorkorderUpdatedV1` is unchanged and remains the **union** of the two: the technician of record first, then any planned id not already named. Consumers must not read its first element as the technician of record — that was this defect, one boundary out.
