...
- That open window is the position's. Technician **assign and reassign** are narrower: they need APPROVED, ASSIGNED or WORK_IN_PROGRESS, so a DRAFT workorder can be dispatched to a bay but not yet handed to a technician (400 if you try). Release is allowed wherever the workorder is still open.
- A workorder has **at most one current technician**. Assign is for the first one, reassign changes hands, release takes the technician off without naming a replacement. Both rules are enforced by database constraints, not only by the service, so two simultaneous dispatchers cannot both win.
- On `assignServicePosition`, a `BAY` or `MOBILE_UNIT` must be **active** (durion-positivity-backend#2001): a bay out of service or a mobile unit that is not deployed is 422 `SERVICE_POSITION_INACTIVE`, and the workorder keeps the position it had.
...
- The UI should read the status back rather than predicting it, and must not show "assigned" for a workorder nobody holds.
- **The dispatch board names one holder and one plan** (durion-positivity-backend#2058). On `WorkorderSummary`, `assignedMechanicId` is the **technician of record** — the workorder's current `technician_assignment` row — and is `null` when the aggregate holds nobody. It never falls back to the legacy `mechanic_ids` column, which it used to: a workorder whose only mechanic arrived through the assignment-context event or the dispatch override showed as held, and `reassignTechnician` then answered 409 `TECHNICIAN_NOT_ASSIGNED` for a row the board had just labelled "assigned to M". Key assign-vs-reassign off this field; a non-null value is exactly "a current technician exists", which is one half of `ASSIGNED` and not `ASSIGNED` itself, so an `APPROVED` workorder may legitimately carry one (technician, no bay yet).
- `plannedMechanicIds[]` on the same summary is that legacy column (#1658), stated honestly: who is expected to work the job. Its two live writers are the shopmgmt-sourced assignment-context event on `workorder.commands.v1` (`WorkorderServiceImpl.handleAssignmentUpdated`) and `overrideOperationalContext`; pos-shop-manager publishes no assignment fact today, so in practice the dispatch override is the one that fills it. It is **scheduling intention, never custody** (DECISION-INVENTORY-021, DECISION-INVENTORY-022) — unvalidated against the person and staffing replicas, not a crew model, and no authority for any technician endpoint. It is published because a dispatcher should see who a job is penciled in for, and because a planned mechanic who is on PTO, at the wrong site or missing a required certification still raises a dashboard conflict. The `mechanicIds` array on `workorder.events.v1`'s `WorkorderUpdatedV1` is unchanged and remains the **union** of the two: the technician of record first, then any planned id not already named. Consumers must not read its first element as the technician of record — that was this defect, one boundary out.
- `MechanicStatus.assignedWorkorderId` on the people panel follows the same rule: the workorder this person holds as its technician of record, `null` otherwise. A mechanic merely planned onto a job does not appear as working it, so the two panels of one board cannot disagree.
