...
#### StorageLocationTopologyResponse.type
...
This domain exposes **39** REST API endpoints:

| Method | Path | Summary |
|--------|------|---------|
| GET | `/v1/locations` | Get all locations |
| POST | `/v1/locations` | Create a new location |
| POST | `/v1/locations/bulk-ingest` | Bulk ingest records |
...
| GET | `/v1/locations/{locationId}/defaults` | Get site defaults |
| PUT | `/v1/locations/{locationId}/defaults` | Configure site defaults |
| GET | `/v1/locations/{locationId}/descendants` | Get all descendants for a location |
...
| GET | `/v1/locations/{siteId}/storage-locations` | List storage locations |
| POST | `/v1/locations/{siteId}/storage-locations` | Create storage location |
| GET | `/v1/locations/{siteId}/storage-locations/topology` | Get storage location topology |
| GET | `/v1/locations/{siteId}/storage-locations/{storageLocationId}` | Get storage location |
| PATCH | `/v1/locations/{siteId}/storage-locations/{storageLocationId}` | Patch storage location |
...
| GET | `/v1/storage-locations/{storageLocationId}/validation` | Validate storage location reference |
...
**Operation ID:** `getDescendants`

**Parameters:**

- `locationId` (path, Required, string): ID of the root location
- `parentType` (query, Optional, string): Parent relationship type to traverse (defaults to PHYSICAL)

**Responses:**

- `200`: List of descendant locations returned successfully.
- `400`: Invalid parentType value.
- `404`: Location not found.
...
- `201`: Storage location created


---

#### GET /v1/locations/{siteId}/storage-locations/topology
...
- `200`: Storage location topology returned
- `404`: Site not found
...
#### GET /v1/storage-locations/{storageLocationId}/validation
...
**Operation ID:** `validateStorageLocation`

**Parameters:**

- `storageLocationId` (path, Required, string): Storage location identifier

**Responses:**

- `200`: Validation payload returned
- `400`: Invalid storageLocationId format
- `403`: Forbidden - missing location:read authority
...
|-------|------|----------|-------------|
| `maxConcurrentVehicles` | integer (int32) | Yes | Maximum number of vehicles that can be serviced concurrently in the bay |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `bayType` | string | No | Type classification of the bay |
| `capacity` | string | No |  |
| `maxConcurrentVehicles` | integer (int32) | No | Maximum number of vehicles that can be serviced concurrently in the bay |
| `name` | string | No | Display name of the bay |
| `serviceCapabilityIds` | array | No | Identifiers of service capabilities supported by the bay |
| `skillRequirementIds` | array | No | Identifiers of skills required to operate the bay |
| `status` | string | No | Operational status of the bay |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `bayType` | string | Yes | Type classification of the bay |
| `capacity` | string | Yes |  |
| `maxConcurrentVehicles` | integer (int32) | No | Maximum number of vehicles that can be serviced concurrently in the bay |
| `name` | string | Yes | Display name of the bay |
| `serviceCapabilityIds` | array | No | Identifiers of service capabilities supported by the bay |
| `skillRequirementIds` | array | No | Identifiers of skills required to operate the bay |
| `status` | string | No | Operational status of the bay |
...
|-------|------|----------|-------------|
| `bayType` | string | No | Type classification of the bay |
| `createdAt` | string (date-time) | No | Timestamp when the bay was created (ISO 8601) |
| `id` | string (uuid) | Yes | Unique identifier of the bay |
| `lastModifiedAt` | string (date-time) | No | Timestamp when the bay was last modified (ISO 8601) |
| `locationId` | string (uuid) | Yes | Identifier of the location that owns the bay |
| `maxConcurrentVehicles` | integer (int32) | No | Maximum number of vehicles that can be serviced concurrently in the bay |
| `name` | string | Yes | Display name of the bay |
| `serviceCapabilityIds` | array | No | Identifiers of service capabilities supported by the bay |
| `skillRequirementIds` | array | No | Identifiers of skills required to operate the bay |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `maxDistance` | number | No | Maximum service distance in kilometres covered by the rule |
| `priority` | integer (int32) | No | Evaluation priority of the rule (lower is evaluated first) |
| `ruleType` | string | Yes | Type of coverage rule |
| `serviceAreaId` | string (uuid) | Yes | Identifier of the service area this rule applies to |
| `validFrom` | string (date) | No | Date from which the rule is effective |
| `validTo` | string (date) | No | Date until which the rule is effective |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `id` | string (uuid) | Yes | Unique identifier of the coverage rule |
| `maxDistance` | number | No | Maximum service distance in kilometres covered by the rule |
| `mobileUnitId` | string (uuid) | Yes | Identifier of the mobile unit this rule belongs to |
| `priority` | integer (int32) | No | Evaluation priority of the rule (lower is evaluated first) |
| `ruleType` | string | No | Type of coverage rule |
| `serviceAreaId` | string (uuid) | Yes | Identifier of the service area this rule applies to |
| `validFrom` | string (date) | No | Date from which the rule is effective |
| `validTo` | string (date) | No | Date until which the rule is effective |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `baseLocationId` | string (uuid) | No | Identifier of the base location the mobile unit operates from |
| `id` | string (uuid) | Yes | Unique identifier of the mobile unit |
| `name` | string | No | Display name of the mobile unit |
| `priority` | integer (int32) | No | Coverage priority of the mobile unit for the requested area (lower is preferred) |
