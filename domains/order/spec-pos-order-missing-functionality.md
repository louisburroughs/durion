...
| --- | --- | --- | --- |
| Q1 | Counter-sale invoice API in pos-invoice: new endpoint, or generalize `InvoiceFinalizationService`? Who owns the story? | §4, §7.2 | Prefer a new counter-sale invoice creation endpoint in pos-invoice, backed by the same lower-level invoice assembly primitives where reuse is real. `InvoiceFinalizationService` is workorder-oriented by name and contract, so generalizing it now would blur authority and pull workorder assumptions into counter-sale flow. Ownership should stay with pos-invoice for API shape and invoice lifecycle semantics, with a paired dependency story in pos-order for the `InvoicingPort` adapter and checkout handshake. |
| Q2 | Payment-settled signal: consume pos-invoice Kafka events (preferred, matches rails) or synchronous confirmation callback? Does pos-invoice publish settlement events today? | §4.2, R12.2 | Prefer Kafka settlement events as the system contract. That matches the outbox/`ProcessedEvent` rails already called out in §12 and preserves pos-invoice as payment authority while keeping checkout synchronous only for invoice creation. Until verified in planning whether pos-invoice already emits settled/reversed events, treat event publication as an explicit dependency; do not design around a synchronous callback as the primary path. A callback may exist only as a temporary compatibility bridge if planning discovers a gap. |
| Q3 | Is `QUOTED` worth keeping given pos-workorder Estimates? (Counter-quote for parts without a vehicle/workorder is the remaining use case.) | §2.1 R2.3 | Yes — keep `QUOTED`, but narrowly scoped to counter-sales and non-workorder parts quotes. pos-workorder Estimates remain authoritative for repair/work flow, while `QUOTED` covers the real walk-in counter case without forcing a workorder-shaped artifact. Planning should explicitly prohibit using `QUOTED` as a shadow estimate for workorder scenarios so the two concepts do not drift into duplicate quote systems. |
| Q4 | RegisterSession placement ratification (pos-order sub-domain vs pos-shop-manager vs new module) and whether shop-manager needs session awareness (clerk shift ↔ schedule). | §6 | Ratify `RegisterSession` inside pos-order as a new sub-domain. The capability is operationally coupled to checkout, tender state, cash movements, and order completion rather than bay scheduling. pos-shop-manager may consume session facts later for clerk-shift visibility, but it should not own drawer-accountability semantics. Given the pre-production policy, a separate module is premature unless multiple bounded contexts later need to mutate register sessions directly. |
| Q5 | Deposit application mechanics in pos-invoice (credit on final invoice) — does `InvoiceAdjustment` cover it? | §7.4 | Treat deposits as a dedicated invoice-level credit application concept, not as a generic adjustment by default. `InvoiceAdjustment` may be a usable implementation primitive if it already preserves source deposit identity, remaining balance, and application auditability, but planning should not assume that without verification. The requirement is that a deposit remains traceable as a liability/credit artifact that can later be applied to a final invoice without losing provenance. |
| Q6 | Returns of workorder-consumed parts: pos-warranty vs pos-order return-order boundary. | §5.4 | Boundary recommendation: pos-order owns returns only for counter-sale lines and for workorder-settled lines explicitly marked returnable at settlement time; warranty or remediation returns for parts already consumed in execution belong to pos-warranty / workorder flows. This keeps pos-order aligned to sale-document reversal, not service-remediation policy. Planning should require an explicit `returnable` signal on imported workorder-backed lines so there is no inferred behavior. |
