...
public class DisasterRecoveryManager {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                ...
                if (validation.isSuccessful()) {
                    return DisasterRecoveryResult.success(recoveryTime, validation.getRecoveredAgents());
                } else {
    ...
    private RecoveryExecution executeRecoveryPlan(DisasterAssessment assessment, DisasterRecoveryOptions options) {
        ...
        for (RecoveryStep step : plan.getSteps()) {
            try {
                executeRecoveryStep(step, assessment.getAffectedAgents(), options);
    ...
    private BackupRecord createAgentBackup(DeployedAgent agent, Instant backupTime) {
        ...
        return new BackupRecord(
            ...
            agent.getEffectiveConfiguration(),
            backupTime
        );
    }
    ...
    private void initializeDefaultRecoveryPlans() {
        ...
        RecoveryPlan agentFailurePlan = new RecoveryPlan(
            "AGENT_FAILURE",
            Arrays.asList(
                new RecoveryStep(RecoveryStepType.STOP_AGENTS, Collections.emptyList()),
                new RecoveryStep(RecoveryStepType.RESTORE_FROM_BACKUP, Collections.emptyList()),
                new RecoveryStep(RecoveryStepType.VALIDATE_HEALTH, Collections.emptyList())
            )
        );
        ...
        RecoveryPlan infraFailurePlan = new RecoveryPlan(
            ...
            Arrays.asList(
                new RecoveryStep(RecoveryStepType.NOTIFY_STAKEHOLDERS, Collections.emptyList()),
                new RecoveryStep(RecoveryStepType.REDEPLOY_AGENTS, Collections.emptyList()),
                new RecoveryStep(RecoveryStepType.VALIDATE_HEALTH, Collections.emptyList())
            )
        );
    ...
    public enum DisasterType {
        AGENT_FAILURE, INFRASTRUCTURE_FAILURE, DATA_CORRUPTION, NETWORK_PARTITION
    }
    ...
    public enum RecoveryStepType {
        STOP_AGENTS, RESTORE_FROM_BACKUP, REDEPLOY_AGENTS, VALIDATE_HEALTH, NOTIFY_STAKEHOLDERS
    }
