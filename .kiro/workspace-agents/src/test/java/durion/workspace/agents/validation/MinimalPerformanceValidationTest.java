...
public class MinimalPerformanceValidationTest {
        ...
        for (int i = 0; i < totalRequests; i++) {
            ...
            testExecutor.submit(() -> {
                try {
                    ...
                    requestMetrics.add(new RequestMetric("avail-req-" + requestId, responseTime, success, Instant.now()));
                } finally {
                    latch.countDown();
                }
            });
                ...
                try {
                    ...
                    if (currentUsers <= MAX_CONCURRENT_USERS) {
                        ...
                        for (int req = 0; req < 5; req++) {
                            ...
                            requestMetrics.add(new RequestMetric("user-" + userId + "-req-" + req, responseTime, true, Instant.now()));
                            Thread.sleep(50);
                        }
    ...
    void testWorkspaceGrowthScalabilityValidation() {
        ...
        assertTrue(meetsGrowthTolerance,
            String.format("Workspace should handle %.1f%% growth without significant performance impact", growthPercentage));
                ...
                try {
                    ...
                    for (int req = 0; req < requestsPerUser; req++) {
                        try {
                            ...
                            requestMetrics.add(new RequestMetric("comp-" + userId + "-" + req, responseTime, success, Instant.now()));
