#!/usr/bin/env python3
"""
Generate BACKEND_API_REFERENCE.generated.md files from OpenAPI specs.

This script parses OpenAPI JSON specs from pos-* modules and generates
generated backend API reference documents for each domain.
"""
...
def resolve_openapi_spec_path(backend_root: Path, module: str) -> Optional[Path]:
    """Resolve OpenAPI spec path for a module (prefer YAML)."""
    candidates = [
...
This document is the generated API reference for the {domain_title} domain.
...
This generated reference summarizes API structures for the {domain_title} domain:
...
|---------|------|---------|
| 1.0 | {today} | Generated API reference from OpenAPI spec |
...
def generate_reference_for_module(backend_root: Path, workspace_root: Path, module: str):
    ...
    spec_path = resolve_openapi_spec_path(backend_root, module)
    if not spec_path:
        print(f"Warning: OpenAPI spec not found for module {module}")
        return
    ...
    reference_content = generate_api_reference_content(
        domain=domain,
        module=module,
        spec=spec,
    ...
    output_path = output_dir / 'BACKEND_API_REFERENCE.generated.md'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate BACKEND_API_REFERENCE.generated.md from OpenAPI specs."
    )
    parser.add_argument(
        "--module",
        action="append",
        dest="modules",
        help="Specific module(s) to generate (repeatable), e.g. --module pos-inventory",
    )
    ...
    for module in modules_with_specs:
        try:
            generate_reference_for_module(backend_root, workspace_root, module)
        except Exception as e:
    ...
    print()
    print("Next steps:")
    print("1. Review generated API references for accuracy")
    print("2. Ensure BACKEND_CONTRACT_GUIDE.md remains curated and behavior-focused")
    print("3. Run validator: scripts/validate_backend_contract_guides.py")
