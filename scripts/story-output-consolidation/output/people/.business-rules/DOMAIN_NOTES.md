...
## New consolidated story insights (non-normative)

This section captures exploratory rationale and institutional memory derived from consolidated frontend story work (e.g., Issue #130). It is intentionally non-normative and may precede final API/contract decisions.

### Story-driven pressure points identified

#### 1) Approval granularity mismatch: pay period vs day/work order

- Existing decision **DECISION-PEOPLE-006** establishes **period-atomic approvals**.
- Issue #130 proposes manager approval flows by **day** and/or **work order**, including batch approve/reject of selected entries.

**Why this matters:**  
If the backend remains period-atomic, a UI that “approves a day” is either:

- a *filtering convenience* that still triggers a period-level approval under the hood, or
- a *new approval mode* that conflicts with the period-atomic invariant.

**Institutional memory / likely intent:**  
The period-atomic model was chosen to align with payroll batching and auditability. The day/work-order view is still valuable as a *review lens* (queueing, triage), but it should not silently change the approval unit without an explicit decision.

**Risk:**  

- Introducing day/work-order approvals without a clear model can create partial approvals within a pay period, complicating payroll export and audit trails.
- If we allow partial approvals, we must define how payroll consumes “partially approved” periods and how rejections interact.

**Mitigation options (exploratory):**

- Keep **period-atomic** as the only state transition, but allow UI to filter entries by day/work order and then approve the entire period-employee set (with clear UX copy).
- Introduce a new concept: **Submission Batch** (e.g., “daily submission” or “work-order submission”) as an intermediate unit, and then payroll approval remains period-atomic. This adds complexity but can reconcile workflows.
- If partial approvals are required, introduce explicit “locked” semantics at the entry level and define payroll export rules (high risk; requires new normative decision).

#### 2) “Locking” semantics after approval

Issue #130 emphasizes “locks approved labor time for payroll accuracy” and “prevents retroactive changes without audit.”

**Existing alignment:**  

- DECISION-PEOPLE-006 already implies approvals are immutable records and entries are updated to approved status.
- DECISION-PEOPLE-017 (optimistic concurrency) and DECISION-PEOPLE-018 (error schema) support safe admin workflows.

**Gap to clarify:**  

- What “locked” means operationally: does it prevent *any* edits, or only edits without an adjustment record?
- Whether “locking” is enforced by People domain only, or also by upstream producers (e.g., work session systems).

**Risk:**  

- If upstream systems can still emit corrected sessions after approval, People must decide whether to:
  - reject ingestion updates,
  - create exceptions,
  - or create adjustments automatically.

**Mitigation (exploratory):**

- Treat approved time entries as immutable; corrections must be represented as **adjustments** (append-only) rather than edits.
- Emit events on approval to downstream consumers; downstream should treat approved entries as final unless an adjustment event arrives.

#### 3) Exceptions as a gating mechanism for approval

Issue #130 introduces **time exceptions** with severity (`WARNING|BLOCKING`) and lifecycle (`OPEN|ACKNOWLEDGED|RESOLVED|WAIVED`), and requires blocking exceptions to be resolved/waived before approval.

**Why this is a new pattern:**  
Existing decisions cover approvals and breaks, but not a generalized exception framework for timekeeping.

**Risk:**  

- If exceptions are computed in another domain (e.g., policy/config), People must avoid becoming a policy engine.
- If exceptions are mutable, we need audit trails for exception state changes (who waived, why).

**Mitigation (exploratory):**

- Model exceptions as People-owned records but allow detection rules to be configured externally (policy/config domain) and delivered as reference data.
- Require waiver notes for `WAIVED` transitions (especially for `BLOCKING`) to satisfy audit/compliance expectations.

#### 4) Adjustments as append-only deltas (do not edit original entry)

Issue #130 proposes creating a separate **adjustment record** (delta/proposed times) without editing the original entry.

**Why this aligns with prior philosophy:**  

- Append-only audit trails are already a theme (DECISION-PEOPLE-006).
- Avoiding mutation of approved records reduces audit risk and supports reconciliation.

**Risk:**  

- If adjustments can be created pre-approval, we must define whether approval applies to:
  - the original entry only,
  - the entry + all proposed adjustments,
  - or requires adjustment approval first.

**Mitigation (exploratory):**

- Define adjustment lifecycle explicitly (e.g., `PROPOSED|APPROVED|REJECTED`) and whether adjustments are included in payroll totals.
- Consider requiring adjustments to be decided before approving the period/entry, or treat them as post-approval corrections with separate audit.

#### 5) Capability/permission gating for exception actions and adjustments

Issue #130 asks for capability flags/permission codes for:
...
- DECISION-PEOPLE-010 and DECISION-PEOPLE-013 already establish permission-based gating and backend-provided capabilities.

**Risk:**  

- UI may hardcode assumptions (e.g., “manager role can waive”) and drift from backend policy.

**Mitigation (exploratory):**

- Backend should return `allowedActions` per entry/exception/adjustment, not just global permissions, to handle nuanced policy (e.g., can approve but cannot waive).

#### 6) Timezone semantics for “day” filtering

Issue #130 highlights ambiguity: display timestamps in user timezone, but day-bucket filtering may need shop/location timezone.

**Existing alignment:**  

- DECISION-PEOPLE-015 defines display rules, but does not fully define “workDate” bucketing semantics.

**Risk:**  

- A “workDate=YYYY-MM-DD” filter can produce inconsistent results if interpreted in user timezone vs location timezone.
- Cross-midnight shifts and DST transitions can cause off-by-one-day errors.

**Mitigation (exploratory):**

- Define `workDate` as a **location-local date** derived from location timezone at time of work (or at time of entry creation).
- Keep stored instants in UTC; store `work_date_local` + `work_date_tz` (or location_id reference) for stable bucketing.

---

## Decision Log (living; non-normative entries)

> This log captures newly identified patterns from stories. These entries may later be promoted into normative decisions in `AGENT_GUIDE.md`.

### DECISION-PEOPLE-027 (Proposed) - Time entry approval queue views vs approval atomicity

- **Status:** Proposed (story-driven; not yet normative)
- **Context:** Issue #130 requires manager review/approval by day and/or work order, while DECISION-PEOPLE-006 defines period-atomic approvals.
- **Proposed decision:** Support **day/work-order filtering as a read/query concern** (“approval queue views”), but keep the **approval transaction unit explicit** (period-atomic unless a new normative decision changes it).
- **Tradeoffs:**
  - Pros: Preserves payroll-aligned audit model; enables manager UX improvements without changing core invariants.
  - Cons: UX must clearly communicate what will be approved (may be more than the filtered subset).
- **Rejected alternatives (for now):**
  - Allow partial approvals by day/work order without a new domain model (rejected due to audit/payroll ambiguity).
- **Risks & mitigations:**
  - Risk: Managers expect “approve selected rows” to only approve those rows.
  - Mitigation: Backend returns `approvalScope` metadata (e.g., `PERIOD_EMPLOYEE`) and UI copy reflects it; or backend supports true row-level approval only if explicitly decided.

### DECISION-PEOPLE-028 (Proposed) - Time exceptions as first-class, auditable gating artifacts

- **Status:** Proposed
- **Context:** Issue #130 introduces exception badges and requires resolving/waiving blocking exceptions before approval.
- **Proposed decision:** Represent timekeeping exceptions as first-class records with:
  - severity (`WARNING|BLOCKING`)
  - status (`OPEN|ACKNOWLEDGED|RESOLVED|WAIVED`)
  - audit fields (who/when/notes)
  - explicit linkage to time entry and/or day scope
- **Tradeoffs:**
  - Pros: Clear audit trail; supports compliance; enables consistent UI gating.
  - Cons: Requires defining ownership of detection rules; adds data model and lifecycle complexity.
- **Rejected alternatives (for now):**
  - Compute exceptions only at read-time with no persistence (rejected due to inability to audit waivers/resolutions).
- **Risks & mitigations:**
  - Risk: People domain becomes a policy engine.
  - Mitigation: Keep detection rules configurable outside People; People stores outcomes and decisions.

### DECISION-PEOPLE-029 (Proposed) - Adjustments are append-only and never mutate original time entries

- **Status:** Proposed
- **Context:** Issue #130 requires “create a separate adjustment record … without editing the original entry.”
- **Proposed decision:** Time entry corrections are represented as **adjustments** (append-only), not edits, especially after approval/lock.
- **Tradeoffs:**
  - Pros: Strong auditability; avoids retroactive mutation; supports reconciliation and downstream eventing.
  - Cons: Payroll/export logic must incorporate adjustments; UI must explain “base vs adjusted” totals.
- **Rejected alternatives (for now):**
  - Allow managers to directly edit submitted/approved entries (rejected due to audit and concurrency risk).
- **Risks & mitigations:**
  - Risk: Confusion about which numbers are “final.”
  - Mitigation: Provide computed “effective time” views and explicit audit panels; emit adjustment events.

### DECISION-PEOPLE-030 (Proposed) - Capability-driven UI actions for time approvals/exceptions/adjustments

- **Status:** Proposed
- **Context:** Issue #130 needs gating for approve/reject/adjust/waive actions.
- **Proposed decision:** For timekeeping approval screens, backend should return **capability flags** (e.g., `allowedActions`) at:
  - queue level (can view, can batch approve)
  - entry level (can approve/reject/create adjustment)
  - exception level (can acknowledge/resolve/waive)
- **Tradeoffs:**
  - Pros: Prevents UI from encoding policy; supports nuanced rules (e.g., can approve but cannot waive).
  - Cons: More backend work; requires consistent contract design.
- **Rejected alternatives (for now):**
  - UI infers capabilities from role names (rejected; violates DECISION-PEOPLE-013).
- **Risks & mitigations:**
  - Risk: Capability drift across endpoints.
  - Mitigation: Centralize capability computation in People domain; add contract tests.

### DECISION-PEOPLE-031 (Proposed) - WorkDate bucketing uses location timezone, display uses user timezone

- **Status:** Proposed
- **Context:** Issue #130 needs day-based filtering and timezone clarity.
- **Proposed decision:** Define `workDate` filters as **location-local dates** (derived using location timezone), while displaying instants per DECISION-PEOPLE-015 (user timezone preferred).
- **Tradeoffs:**
  - Pros: Stable “day” semantics for shop operations; avoids cross-timezone confusion for location-scoped work.
  - Cons: Requires location timezone availability and careful DST handling.
- **Rejected alternatives (for now):**
  - Interpret `workDate` in user timezone (rejected due to inconsistent results for managers operating across locations).
- **Risks & mitigations:**
  - Risk: Cross-midnight sessions split across days.
  - Mitigation: Define bucketing rule (e.g., bucket by start time local date) and document it; consider exceptions for long sessions.

---

## Known Risks / Mitigations (timekeeping approvals expansion)

- **Domain boundary risk (People vs WorkExec):**
  - Story explicitly notes a domain conflict label; timekeeping is People-owned.
  - **Mitigation:** Enforce routing/triage rules: timekeeping approval/locking/adjustments/exceptions remain People domain; WorkExec consumes read-only availability/time outputs.

- **Eventual consistency risk (approval outputs to payroll/HR):**
  - Approvals likely produce events; UI should not synchronously depend on downstream HR.
  - **Mitigation:** Keep UI success criteria local to People write; provide audit metadata and correlation IDs for traceability.

- **Concurrency risk (multiple managers approving):**
  - Story expects 409 on conflict.
  - **Mitigation:** Apply DECISION-PEOPLE-017 concurrency tokens to time entry/approval mutations; return per-item failure details when batch operations are used.

- **Audit/compliance risk (waivers and adjustments):**
  - Waiving blocking exceptions and creating adjustments are high-audit actions.
  - **Mitigation:** Require reason/notes for waive and reject; store immutable decision records; ensure all actions are attributable (who/when).

---

## Open Questions (updated from consolidated stories)
...
8. **Approval atomicity vs UI batch selection (blocking/clarification):** If UI allows selecting a subset of entries (e.g., for a day/work order), does backend:
   - approve only selected entries (implies partial approvals), or
   - treat selection as a convenience but approve at period-employee scope (per DECISION-PEOPLE-006)?  
9. **Locking semantics (clarification):** After approval, are entries strictly immutable, or can they be corrected only via adjustments? Are there any HR-admin override capabilities (similar to DECISION-PEOPLE-025 terminated edit rules)?  
10. **Exception waiver requirements (clarification):** Is `resolutionNotes` required for `WAIVED` (especially for `BLOCKING`)? Are waiver reason codes needed?  
11. **Adjustment lifecycle (clarification):** Are adjustments immediately effective for payroll totals, or do they require separate approval? If separate approval exists, who can approve and what permission governs it?  
12. **Batch operation error reporting (clarification):** For `POST /approve` and `POST /reject`, does the API return per-item results (success/failure) to support partial failures and better UX?  
