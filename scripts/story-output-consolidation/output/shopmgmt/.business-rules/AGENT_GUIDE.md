...
- **Appointment creation from source documents** (Estimate/Work Order) is owned by shopmgmt:
  - shopmgmt is authoritative for eligibility enforcement, operating-hours enforcement, conflict detection/classification, idempotency, and immutable linkage to the source document.
  - shopmgmt may expose a **create-screen read model** (e.g., `AppointmentCreateModel`) that includes **PII-safe** source summary + backend-provided defaults/hints; UI must not infer policy-owned fields. (**CLARIFY** exact contract/service names; see Open Questions from Frontend Stories.)
- **Create-from-source UX support contracts** (read models + deterministic error codes):
  - Provide deterministic `422` error codes for ineligibility and policy failures (DECISION-SHOPMGMT-011/013).
...
### AppointmentCreateModel (read model) — **NEW**

Frontend-facing read model for the Appointment Create screen when launched from a source document (Estimate or Work Order). This is a **backend-owned contract** and must be documented before UI implementation (DECISION-SHOPMGMT-011).

Expected fields (from consolidated story #76; treat as contract shape):

- `sourceType`: `ESTIMATE | WORK_ORDER`
- `sourceId`
- `facilityId`
- `facilityTimeZoneId` (IANA TZ, e.g., `America/New_York`)
- `sourceStatus`
- `isEligible`
- `ineligibilityCode?`, `ineligibilityMessage?` (safe)
- `defaults?`:
  - `suggestedDurationMinutes?`
  - `requiredSkills[]?`, `preferredSkills[]?`
  - `serviceSummary?` (**must be PII-safe**)
- `existingAppointmentId?` (if source already linked; **permission-gated server-side**; **CLARIFY** behavior)
...
| AppointmentCreateModel | Frontend read model for create screen launched from Estimate/Work Order (eligibility + facilityTimeZoneId + safe defaults/hints). |
...
- A source document may not have multiple active appointments unless backend explicitly supports it.
  - **CLARIFY:** whether “already linked” returns `existingAppointmentId` in the create model and whether it is permission-gated.
- Conflicts:
  ...
  - Conflicts are returned on submit (create/reschedule). A pre-check may exist for UX but must not be required for correctness.
- Operating hours are enforced server-side; the UI must not infer hours.
  - Outside operating hours / holiday closure is represented as a HARD conflict (DECISION-SHOPMGMT-008).
- Reschedule requires:
  - reason enum
  - notes when reason is OTHER
  - notes and permission when overriding a SOFT conflict or policy exception
- Create-from-source requires:
  - `scheduledStartDateTime` (required)
  - `facilityId` explicit in write requests (DECISION-SHOPMGMT-012)
  - `clientRequestId` for idempotency (DECISION-SHOPMGMT-014)
  - `overrideReason` when overriding SOFT conflicts (DECISION-SHOPMGMT-007)
- Assignment notes:
...
> This section is the running list of unresolved questions that block or materially affect frontend/backend integration.
> Items here should be resolved with concrete repo-specific answers (service names, screen paths, permission strings).

1. **Moqui screen paths and parameter names** (**CLARIFY**) for:
   - Estimate Detail
   - Work Order Detail
   - Appointment Create
   - Appointment Detail  
...
2. **Backend service names/endpoints and payload schemas** (**CLARIFY**) for:
   - Load `AppointmentCreateModel` (or equivalent) from source
   - Create appointment from source (including 409 conflict payload shape, 422 codes, and success response fields)  
   (Required to implement Moqui transitions/forms without guessing; DECISION-SHOPMGMT-011.)

3. **Exact permission identifiers** used by the POS security model (**CLARIFY**; do not guess strings):
   - CREATE_APPOINTMENT
   - OVERRIDE_SCHEDULING_CONFLICT
   - (Existing permissions referenced elsewhere: EDIT_ASSIGNMENT_NOTES, APPROVE_RESCHEDULE, AUDIT_VIEW)  
   (Security boundaries; SAFE_DEFAULTS_MODE denylist.)

4. **Source already linked behavior** (**CLARIFY**):
   - Does the backend return `existingAppointmentId` when a source is already linked?
   - Is `existingAppointmentId` permission-gated?
   - If not returned, what is the expected UI behavior (message only vs link)?

5. **FacilityId requirement for read/load calls** (**CLARIFY**):
   - Does the backend require `facilityId` for the create-model load call as well, or only for writes?
   - If facilityId is omitted on reads, confirm backend still enforces facility authorization and does not leak cross-facility existence (DECISION-SHOPMGMT-012).

## Integration Patterns and Events (Actionable Guidance)

> This section consolidates integration expectations implied by decisions and stories. It does not change decisions; it makes them implementable.

### Synchronous APIs (request/response)

- **Create from source** must be a single submit that returns either:
  - success (200/201) with `appointmentId`, `appointmentStatus`, `scheduledStartDateTime`, `facilityTimeZoneId`, and optional `notificationOutcomeSummary`, or
  - `409 SCHEDULING_CONFLICT` with `conflicts[]` and optional `suggestedAlternatives[]`, or
  - `422` with deterministic `errorCode` for eligibility/policy failures, or
  - `400` for syntactic/field validation.
- **Idempotency**: create/reschedule must accept `clientRequestId` and return the original success response on duplicates (DECISION-SHOPMGMT-014).
  - **TODO**: confirm storage keying strategy: `(facilityId, operationName, clientRequestId)` vs `(operationName, clientRequestId)`.

### Server-Sent Events (SSE) for near-real-time updates

- SSE stream is facility-scoped: `shopmgmt.assignment.facility.<facilityId>` (DECISION-SHOPMGMT-006).
- Event payload should include full `AssignmentView` and a monotonic `version` to allow clients to ignore out-of-order updates.
- **Security**: SSE subscription must enforce facility authorization at connect time; do not rely on client-side filtering.

### Event emission (internal)

- Emit domain events on:
  - appointment created (from estimate/work order)
  - appointment rescheduled
  - assignment changed (bay/mobile/mechanic)
  - assignment notes updated
  - conflict override applied
- **CLARIFY**: whether these are persisted to an outbox for reliable delivery (recommended) or best-effort in-process publish.

## API Contracts and Patterns (Concrete)

- **Error shape** (recommended minimum; **CLARIFY** if repo already standardizes):
  - `errorCode` (string, stable)
  - `message` (safe, user-displayable)
  - `fieldErrors[]` optional: `{ field, message, code? }`
  - `conflicts[]` only for 409: `{ severity, code, message, overridable }`
- **HTTP semantics**:
  - `400` for syntactic validation (missing required fields, invalid datetime format)
  - `422` for business rule/policy failures (eligibility, reschedule approval required, etc.)
  - `409` for scheduling conflicts
  - `403/404` for unauthorized/not found without leaking cross-facility existence (DECISION-SHOPMGMT-012)
- **Correlation**:
  - Require `X-Request-Id` on all calls; propagate to downstream services and include in logs (DECISION-SHOPMGMT-011).

## Security / Authorization Requirements (Deny-by-default)

- All write operations must validate:
  - actor is authenticated
  - actor has required permission(s)
  - actor is authorized for `facilityId` in request (DECISION-SHOPMGMT-012)
- All read operations must validate:
  - actor is authorized for the derived facility of the resource (appointmentId/sourceId)
  - return 403/404 without confirming existence across facilities (DECISION-SHOPMGMT-012)
- **Sensitive fields**:
  - `assignmentNotes`, `overrideReason`, and audit reasons are sensitive operational text.
  - Do not log these fields in plaintext in application logs.
  - Audit storage may contain them, but audit read models must be redacted/permission-gated (DECISION-SHOPMGMT-017).
- **TODO/CLARIFY**: confirm canonical permission names in this repo for:
  - CREATE_APPOINTMENT
  - OVERRIDE_SCHEDULING_CONFLICT
...
> Goal: make scheduling failures diagnosable without logging PII/sensitive text.

### Metrics (backend)

- `shopmgmt_create_appointment_total{sourceType, outcome}` where outcome ∈ `success|conflict|policy|validation|auth|error`
- `shopmgmt_conflict_total{code, severity}`
- `shopmgmt_conflict_override_total{code}` (count overrides; alert on spikes)
- `shopmgmt_idempotency_hit_total{operation}` (detect UI retry/double-submit patterns)
- `shopmgmt_sse_connections{facilityId?}` (**avoid high-cardinality**; prefer aggregate + sampled facility)
- `shopmgmt_sse_event_delivery_latency_seconds` (p50/p95/p99)

### Logs (backend)

- Always include: `X-Request-Id`, `facilityId`, `appointmentId` (when known), `sourceType/sourceId` (IDs only), and `errorCode`.
- Never log: `assignmentNotes`, `overrideReason`, `notesEditReason`, or any customer PII.
- For 409 conflicts: log conflict `code` and `severity` counts, not full messages if messages could embed sensitive context.

### Traces

- Create/reschedule flows should create a trace span that includes:
  - `facilityId`
  - `operation` (`createFromEstimate|createFromWorkOrder|reschedule|updateAssignment|updateNotes`)
  - `outcome` and `errorCode` (if any)
- Propagate `X-Request-Id` into trace correlation if the tracing system supports it.
...
- Create-from-source:
  - 201 success includes required fields and facilityTimeZoneId
  - 422 ineligible estimate returns `ESTIMATE_NOT_ELIGIBLE`
  - 422 ineligible work order returns `WORK_ORDER_NOT_ELIGIBLE`
  - 409 conflict returns `conflicts[]` with `severity` and `overridable`
  - suggestedAlternatives shape is stable when present
- Idempotency:
  - same `clientRequestId` returns same `appointmentId` and does not create duplicates
- Authorization:
  - cross-facility access returns 403/404 without leaking existence
  - override requires permission + non-empty reason
- Concurrency:
  - assignment notes update rejects version mismatch with 409

### Unit tests

- Conflict classification: HARD vs SOFT mapping is deterministic
- Operating hours enforcement: outside hours always yields HARD conflict
- Reschedule policy: count threshold and approval gating

### Integration tests

- Location domain dependency:
  - operating hours retrieval failures: **CLARIFY** expected behavior (fail closed vs fail open). Default should be **fail closed** with a safe error code (TODO: confirm).
- People domain dependency:
  - mechanic display fields degrade gracefully when People is unavailable (DECISION-SHOPMGMT-009).

## Common Pitfalls and Gotchas

- **Timezone bugs**: UI must not send naive datetimes; always send ISO-8601 with offset and display using `facilityTimeZoneId` (DECISION-SHOPMGMT-015).
- **Double-submit duplicates**: without `clientRequestId` persistence across retries, users can create duplicate appointments. UI must reuse the same `clientRequestId` after timeouts (DECISION-SHOPMGMT-014).
- **Leaking existence across facilities**: returning different errors for “not found” vs “not authorized” can leak data. Use consistent 403/404 strategy (DECISION-SHOPMGMT-012).
- **Logging sensitive text**: override reasons and notes are operationally sensitive; do not log them client-side or server-side (DECISION-SHOPMGMT-017).
- **Assuming People calls**: Appointment Detail must not require People domain calls to render; mechanic details may be partial (DECISION-SHOPMGMT-009).
- **UI hardcoding status/actions**: treat appointment status as opaque; rely on backend allowedActions when needed (DECISION-SHOPMGMT-013).
...
- **NEW todo (from consolidated story #76):** "CLARIFY create-from-source read model + service names" → TODO: TASK-SHOP-002 (document Moqui service names, request/response schemas for AppointmentCreateModel load + create submit) | Decision: DECISION-SHOPMGMT-011
- **NEW todo (from consolidated story #76):** "CLARIFY Moqui screen paths/params" → TODO: TASK-SHOP-003 (document screen paths + required params for Estimate Detail, Work Order Detail, Appointment Create, Appointment Detail) | Decision: DECISION-SHOPMGMT-011
- **NEW todo (from consolidated story #76):** "CLARIFY permission strings" → TODO: TASK-SHOP-004 (confirm exact permission identifiers in repo for CREATE_APPOINTMENT and OVERRIDE_SCHEDULING_CONFLICT) | Decision: DECISION-SHOPMGMT-012/007
