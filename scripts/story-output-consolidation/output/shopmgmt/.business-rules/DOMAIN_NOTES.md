...
This is **non-normative** documentation: it captures *why* we made choices, tradeoffs, and institutional memory. Normative requirements live in `AGENT_GUIDE.md` and service contracts.
...
## Consolidated story insights (institutional memory)

This section captures cross-cutting rationale surfaced by consolidated stories (e.g., Issue #76) and how they reinforce or stress existing decisions. It is intentionally exploratory.

### Create-from-source is a “policy enforcement choke point”

Issue #76 reinforces that **appointment creation from Estimate/Work Order is not a thin CRUD operation**. It is the point where we must enforce:

- **Eligibility** (DECISION-SHOPMGMT-013): estimate/work order status gating.
- **Facility scoping** (DECISION-SHOPMGMT-012): explicit facilityId on writes; deny-by-default on reads.
- **Timezone semantics** (DECISION-SHOPMGMT-015): facility timezone is authoritative; UI must not guess.
- **Operating hours** (DECISION-SHOPMGMT-008): outside-hours is a HARD conflict.
- **Conflict classification and override** (DECISION-SHOPMGMT-002/007): HARD blocks; SOFT requires permission + reason + audit.
- **Idempotency** (DECISION-SHOPMGMT-014): retries must not create duplicates.

**Tradeoff:** This concentrates complexity in one endpoint/flow, but it reduces “policy drift” across multiple UI screens and prevents inconsistent scheduling behavior.

### “No pre-check conflicts required” is a deliberate UX/architecture choice

The story explicitly calls out that a “pre-check conflicts” call is optional and not required for correctness. This aligns with DECISION-SHOPMGMT-011 and DECISION-SHOPMGMT-002:

- Correctness comes from **submit-time conflict detection**.
- Pre-check endpoints can improve UX but introduce:
  - race conditions (availability changes between pre-check and submit),
  - additional contract surface area,
  - higher load and caching complexity.

**Mitigation if we add pre-check later:** treat it as advisory only; always re-check on submit; ensure error semantics remain stable (409 on submit).

### Suggested alternatives are intentionally constrained

Issue #76 expects `suggestedAlternatives[]` to be **time slots only**. This is an important boundary:

- We avoid the system “auto-assigning” bays/mechanics implicitly during create.
- We keep the create flow focused on *time selection* and policy enforcement.

**Rejected alternative (implicit):** returning “suggested bay/mechanic assignments” during create.

- Pros: faster scheduling.
- Cons: couples create flow to assignment optimization, increases People/Location dependencies, and risks leaking sensitive staffing constraints.

### Free-text operational fields are treated as sensitive by default

The story reiterates: do not log `overrideReason` client-side. This is consistent with DECISION-SHOPMGMT-017.

**Institutional memory:** operational free-text often contains accidental PII or sensitive internal commentary. Even if we *intend* it to be non-PII, we should treat it as sensitive and minimize propagation.

---

## Risk assessment & mitigations (updated)

### Risk: Duplicate appointments due to retries/double-submits

- **Why it matters:** Create-from-source is a high-frequency workflow; UI timeouts are common.
- **Mitigation:** DECISION-SHOPMGMT-014 idempotency with `clientRequestId` and stable replay of success response.
- **Residual risk:** if clients generate new IDs on retry, duplicates can still occur.
  - **Mitigation:** UI guidance: generate once per user-initiated submit attempt and reuse until terminal outcome.

### Risk: Cross-facility data leakage (existence probing)

- **Why it matters:** “Create from estimate/work order” entry points can be used to probe IDs.
- **Mitigation:** DECISION-SHOPMGMT-012 deny-by-default; return 403/404 without confirming existence; ensure error messages are generic.
- **Residual risk:** timing side-channels (fast 404 vs slow 403).
  - **Mitigation:** normalize error handling paths where feasible; add rate limiting at gateway.

### Risk: Policy drift between UI and backend

- **Why it matters:** UI might hardcode eligibility statuses, conflict messages, or operating hours.
- **Mitigation:** DECISION-SHOPMGMT-011 contract-first; DECISION-SHOPMGMT-013 backend-authoritative status; UI uses backend-provided safe messages.
- **Residual risk:** UI still needs some gating for usability.
  - **Mitigation:** treat UI gating as advisory; always rely on server enforcement.

### Risk: Dependency availability (Location/People/Notification)

- **Why it matters:** create flow depends on Location for operating hours; People for display; Notification for side effects.
- **Mitigation:** DECISION-SHOPMGMT-008: treat Location as authoritative; if unavailable, decide whether to fail closed (block scheduling) vs fail open (dangerous).
- **Mitigation:** DECISION-SHOPMGMT-016: notification failures should not necessarily fail create; return partial outcome summary.
- **Open design tension:** Location unavailability behavior is not explicitly captured as a decision (see Open Questions).

### Risk: Overriding SOFT conflicts becomes routine (“rubber stamping”)

- **Why it matters:** overrides can hide staffing/capacity problems.
- **Mitigation:** DECISION-SHOPMGMT-007 structured audit + monthly review; DECISION-SHOPMGMT-002 monitoring on override rate.

---

## Decision Log (non-normative additions)

These entries capture patterns surfaced by stories. They may later become normative decisions, but are recorded here as institutional memory.

### DECISION-LOG-SHOPMGMT-DL-001 — Submit-time conflict detection is the correctness boundary

- **Status:** Observed pattern (reinforces DECISION-SHOPMGMT-002/011)
- **Context:** Issue #76 explicitly treats “pre-check conflicts” as optional.
- **Decision (rationale):** The system must be correct even if the UI never calls a pre-check endpoint. Therefore, conflict detection and operating-hours enforcement must occur on the create submit path and return structured conflicts (409).
- **Tradeoffs:**
  - Pros: single enforcement point; fewer race conditions; simpler contracts.
  - Cons: UX may feel “late failing” without pre-check; requires good conflict payloads and suggested alternatives.
- **Rejected alternative:** require pre-check before submit.
  - Why rejected: creates false sense of safety; still must re-check on submit; doubles API surface.

### DECISION-LOG-SHOPMGMT-DL-002 — Create-from-source read model should be PII-minimized and permission-gated

- **Status:** Observed pattern (reinforces DECISION-SHOPMGMT-017)
- **Context:** Issue #76 requests a backend-provided `AppointmentCreateModel` with “source summary (non-PII where possible)” and optional `existingAppointmentId` that is permission-gated.
- **Decision (rationale):** The create screen should be powered by a backend read model that is safe-by-default:
  - include only what is needed to schedule (facility timezone, eligibility, safe service summary),
  - avoid customer PII unless explicitly authorized,
  - treat “existing appointment linkage” as sensitive (can reveal operational history).
- **Tradeoffs:**
  - Pros: reduces leakage risk; consistent with least privilege.
  - Cons: UI may need additional calls for richer context (which may be blocked by permissions).
- **Rejected alternative:** UI composes create screen by calling Estimate/Work Order domains directly.
  - Why rejected: increases coupling, inconsistent redaction rules, and pushes security decisions into UI.

### DECISION-LOG-SHOPMGMT-DL-003 — Suggested alternatives are time-slot-only (not resource assignments)

- **Status:** Observed pattern (aligns with DECISION-SHOPMGMT-003 and story expectations)
- **Context:** Issue #76 expects `suggestedAlternatives[]` to be time slots only.
- **Decision (rationale):** Alternatives returned from conflict responses should be limited to time windows; resource assignment remains a separate concern.
- **Tradeoffs:**
  - Pros: avoids premature optimization; keeps create flow stable; reduces dependency on People/Location capacity models.
  - Cons: schedulers may still need manual iteration to find a workable bay/mechanic later.

---

## Open Questions / Known Issues (updated)

These are consolidated from Issue #76 and represent current unknowns that block or risk implementation. Keep this section current as contracts and repo conventions are clarified.

1. **Moqui screen paths and parameter names** in this repo for:
   - Estimate Detail
   - Work Order Detail
   - Appointment Create
   - Appointment Detail  
...
2. **Exact backend service names/endpoints and payload schemas** for:
...
3. **Exact permission identifiers** used by the POS security model for:
   - CREATE_APPOINTMENT
   - OVERRIDE_SCHEDULING_CONFLICT  
   (Do not guess permission strings; SAFE_DEFAULTS_MODE denylist: security boundaries.)

4. **Source already linked behavior:** Does the backend return `existingAppointmentId` when a source is already linked, and is that field permission-gated? If not returned, what is the expected UI behavior (message only vs link)?

5. **Facility scoping on read model load:** Does the backend require `facilityId` for the **load** call as well, or only for writes? (DECISION-SHOPMGMT-012 allows reads to derive facilityId but does not mandate it.)

6. **Location service unavailability behavior (operating hours):** When Location cannot be reached or returns incomplete hours/holiday data, do we:
   - fail closed (treat as HARD conflict / block scheduling),
   - fail open (allow scheduling), or
   - degrade (allow but mark as SOFT conflict requiring override)?  
   This is a policy decision with operational and compliance implications; currently implicit in DECISION-SHOPMGMT-008 but not explicitly decided.

7. **Error code taxonomy for create-from-source:** Issue #76 suggests 422 codes like `ESTIMATE_NOT_ELIGIBLE` / `WORK_ORDER_NOT_ELIGIBLE` and 409 `SCHEDULING_CONFLICT`. Confirm the canonical set and whether operating-hours violations are always represented as 409 conflicts vs 422 policy errors (story expects HARD conflict via 409; DECISION-SHOPMGMT-008 frames it as conflict).

8. **Idempotency retention and scope:** DECISION-SHOPMGMT-014 states 24h retention keyed by (clientRequestId, operation). Confirm whether create-from-source uses:
   - operation key = `CREATE_APPOINTMENT_FROM_SOURCE`,
   - and whether the key is additionally scoped by facilityId/sourceId to prevent accidental cross-context reuse.
