...
| 0061 | Location scope authorization: `roles.location_scope` (ALL/LOCATION) says whether, pos-people's `employee_location_assignment` says where (security-service `role_assignments` scope and `check-permission` retired), scope assigned at a node covers its descendants (evaluated at check time against a replicated ancestor set, never expanded into the token), hierarchy dimension is a role property (FINANCIAL for accounting/general-manager roles, OTHER for the rest), additive `loc_fin_bits`/`loc_oth_bits`+discriminated `loc_scope` claims with no `CATALOG_VERSION` bump, owning-service enforcement, `exp` clamped to assignment expiry with revoke-on-change                                             | Planner, Coder, Test, Orchestrator |
