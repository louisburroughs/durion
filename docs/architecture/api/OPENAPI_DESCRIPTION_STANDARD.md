---
type: Architecture
title: OpenAPI Operation Description Standard
description: Fixes the exact sentence order and lead-in phrases a backend @Operation description must use so the MCP server can compare tools, and names the validator checks that enforce it.
status: Current
tags: [openapi, mcp, annotations, adr-0042, pos-openapi-validation, tool-descriptions]
---

An `@Operation` description on a Durion backend controller is a tool-selection prompt read by the
MCP server, not documentation for a human, and this document fixes the wording rules that make two
such descriptions comparable.

It covers the two ADR-0042 requirements that no module satisfied before it existed (see
[#1263](https://github.com/louisburroughs/durion-positivity-backend/issues/1263)):
...
[ADR-0042](../../adr/0042-openapi-annotation-standards.adr.md) states *what* must be
present. This document fixes *how* it is written, so that the 1,104 operations across the 25
spec-publishing modules read as one voice rather than 25 interpretations, and so that the depth can
be machine-checked by `pos-openapi-validation` instead of remaining aspirational.
...
`REPORT_ONLY` and `EXEMPT` remain supported by `OpenApiAnnotationDepthValidator`, but **no module
uses either today**. The inventory holds 27 entries: 25 carry `annotationDepth: STRICT`, and the
remaining two are `mode: EXCEPTION` with a stated reason — `pos-api-gateway` ("aggregate producer
without module paths") and `pos-inquiry` ("stub module with no REST producer surface"). Neither
...
## A new module joins at STRICT

**The rollout is finished.** Every module that publishes a spec is already `annotationDepth:
STRICT`, so there is no staged conversion left to do and no `REPORT_ONLY` grace period to fall
back on. A new module joins the inventory at `STRICT` in the same change that adds its first
controller, with its spec regenerated alongside (`scripts/generate-openapi.sh <module>`).

Practically, this means a module's first controller must ship full seven-element descriptions or
its build fails. That is the intended cost: a module admitted below the standard is a module
nobody goes back to. `pos-tax` and `pos-supplier` were the original reference conversions and are
still the clearest worked examples — read their controllers before writing a new module's first.

An `EXCEPTION` entry (not `EXEMPT`) is the only way out, and it is for modules with **no** operations
at all: it needs a stated `reason` in the inventory, as `pos-api-gateway` and `pos-inquiry` carry.
A module with operations does not qualify.
