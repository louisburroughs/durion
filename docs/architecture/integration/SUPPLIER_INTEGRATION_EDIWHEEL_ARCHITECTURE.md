...
| MKCAT enrichment is published but nothing attaches it to products | [#1352](https://github.com/louisburroughs/durion-positivity-backend/issues/1352) | **SHIPPED** — [PR #1359](https://github.com/louisburroughs/durion-positivity-backend/pull/1359) (2026-08-17) added the pos-catalog attachment model (`TreadDesignEntity`, `product.tread_design_id`), a `supplier.catalog.updated` consumer, a single-threshold trigram matcher, and the `getTreadDesignForProduct` / `listUnmatchedTreadDesigns` reads. [#1645](https://github.com/louisburroughs/durion-positivity-backend/issues/1645) and [ADR-0060](../../adr/0060-catalog-enrichment-matching.adr.md) extend it in place: confidence tiers with a brand gate, ambiguity parking, MANUAL-attachment stickiness, and a new review/resolve contract (`listTreadDesignCandidates`, `resolveTreadDesign`) — no rebuild of what #1352 already delivered |
