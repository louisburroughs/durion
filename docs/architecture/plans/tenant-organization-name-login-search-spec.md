...
-- backfill, then de-duplicate, then constrain (alpha data; suffix the later row by created_at)
-- ... see migration body: UPDATE ... SET display_name_key = lower(btrim(regexp_replace(...)))
--     followed by a DO block appending ' #<n>' to collisions ...
...
**`display_name_key` is written by the application, not generated by the database**, and
`TenantEntity` maps it as an ordinary insertable/updatable column. A Postgres
`GENERATED ALWAYS ... STORED` column would be the tighter backstop, but it is not viable here:

* the `dev` profile runs **H2** with `flyway.enabled: false` and `ddl-auto: create-drop`
  (`pos-tenant/src/main/resources/application-dev.yml`), so Hibernate builds the schema from the
  entity. A database-generated column would simply be absent-and-null under `dev` and under every
  H2-backed test, leaving the collision check querying a column nothing ever populates — the seeding
  logic would silently misbehave in exactly the environments developers run;
* NFKC (step 1 of §4.1) has no Postgres equivalent, so the application must normalize regardless. A
  generated column would define a *second, subtly different* normalization beside it.

One normalizer, in `TenantDisplayNameAllocator`, applied on every write. The unique constraint still
enforces uniqueness in the database — only the normalization is application-side, which is the same
trade every other normalized field in this codebase makes.
...
(`V3__ext_tenant.sql:5`). New migration `V6__ext_tenant_display_name_search.sql` (V5 is the highest
existing) adds a `display_name_key` column, backfilled and maintained by the replica listener on
every upsert — same rule as §4.1, same reason as §5.1 — plus
...
| Rate limit | Per client IP, `429` on breach — enforced at the **gateway**, not in this service (§5.4a). |
...
| Gateway `RequestRateLimiter` on the search path | `60`/min per client IP, burst 10 | `pos-api-gateway` (§5.4a) |
...
### 5.4a Rate limiting — gateway, not the service

An earlier draft said to "reuse the existing login-throttle infrastructure". **There is none that
fits.** `LockoutService` is per-**user** account lockout keyed by `UUID userId` and persisted to the
`users` table (`LockoutService`, `LockoutPolicy`); the search endpoint is anonymous and has no user,
so there is nothing to key on and nothing to write. Rate limiting here is net-new work — budget for
it.

Put it at the gateway, which already has what is needed: `spring-boot-starter-data-redis-reactive`
and a configured Redis it uses for the `jwt:revoked:{jti}` checks
(`pos-api-gateway/src/main/resources/application.yml:8-14`). Add Spring Cloud Gateway's
`RequestRateLimiter` with a `RedisRateLimiter` and a client-IP `KeyResolver`, applied to the search
path on the `security-service` route. Redis-backed means the limit holds across gateway instances,
which an in-process limiter would not.

Two implementation traps, both of which must be covered by a test rather than assumed:

1. **It must fail open.** The gateway's existing policy is explicit — the revocation check fails
   open and the Redis health indicator is disabled on purpose "so a Redis outage must not mark the
   gateway unhealthy and take the whole platform's ingress out of rotation"
   (`application.yml:191-195`). A rate limiter on that same Redis that fails *closed* would turn a
   Redis blip into a total login outage: strictly worse than the enumeration risk it mitigates.
   Verify the configured behaviour on Redis failure; do not assume the framework default.
2. **The client IP must be resolved correctly.** `X-Forwarded-*`/`Forwarded` headers are stripped
   unless the peer matches the trusted-proxy regex (CVE-2025-41235 hardening,
   `application.yml:16-19`). A naive `KeyResolver` reading the remote address will see the load
   balancer's single IP for every request and throttle the whole world into one bucket. The resolver
   must agree with that trusted-proxy configuration; test it with and without a forwarded header.
...
* The backfill and de-duplication steps of `V3__tenant_display_name_key.sql` leave the constraint
  satisfiable on a fixture with three identically named tenants.
* Under the `dev`/H2 profile (schema from entities, no Flyway) the allocator and the collision check
  behave identically to Postgres — the key is populated because the application writes it.
...
* Search reachable without a token; search is enabled by default in every profile, and
  `auth.tenant-search.enabled=false` ⇒ `404` (the incident lever).
* Gateway (§5.4a): the limiter answers `429` past the threshold; it **fails open** when Redis is
  unavailable — a Redis outage must not block login; the `KeyResolver` distinguishes two clients
  behind the load balancer and does not collapse them into one bucket.
...
| 4 | `V6__ext_tenant_display_name_search.sql` + listener upsert of the key + repository query | backend / `pos-security-service` | — |
| 5 | Public search endpoint, config keys, event type | backend / `pos-security-service` | 4 |
| 5a | Gateway `RequestRateLimiter` + IP `KeyResolver`, fail-open verified | backend / `pos-api-gateway` | — |
