...
| First administrator credential (decided 2026-09-10) | **Operator-delivered activation token.** Provisioning leaves the tenant's first `ADMIN` credential-expired; a platform-tenant caller (`platform:tenant:provision`) mints a one-time, 72-hour activation token through a narrow `pos-security-service` endpoint that binds the target tenant explicitly (sanctioned as a provisioning operation), and an unauthenticated `POST /v1/auth/activate {token, password}` sets the password. No mail infrastructure; the same token later drives an e-mail reset |
| Tenant registry behind `TenantIterator` (decided 2026-09-10) | **Cached REST lookup against `pos-tenant`.** The library's `RemoteTenantRegistry` calls an internal, shared-secret `pos-tenant` list endpoint on a refresh interval, keeps the last good snapshot through outages, and starts from the static list. Accepted: a runtime dependency on `pos-tenant` and one service secret; no per-module `ext_tenant` replica |
| Bulk-loaded roles and the template (decided 2026-09-10) | **Loader targets the platform tenant; provisioning reconciles.** `pos-bulk-loader` takes a target tenant per job; a role loaded into the platform tenant gets `template_key` and joins the template; `reconcileTemplate(tenant)` adds newly templated roles and grants to existing tenants without touching tenant-local edits |
...
| WS4 | *Pilot half done with WS1 (Kafka header, consumer interceptor, outbox `tenant_id` data column on `pos-location`); the envelope field and the producer signature change remain.* Async platform: envelope `tenantId`, Kafka header, consumer `RecordInterceptor`, outbox as a global table with `tenant_id` data, producer signature change (31 sites); per-tenant manifests and replay; `RemoteTenantRegistry` + `pos-tenant` internal list endpoint (decisions of 2026-09-10) | backend | 1 - 2 | WS1 |
...
| WS6 | Storage, observability, operations: documents/images tenant-prefixed paths, MDC and trace tenant tag, `pos-mcp-server` session scoping and per-tenant tool-priority overlay, `emitted_event_hourly` by tenant with global rollups (decision of 2026-09-10), per-tenant export tooling for offboarding (replaces per-cell `pg_dump`), Compose/alpha runbook role changes | backend, durion | 2 - 3 | WS1 |
...
| WS8 | Seed and bulk load: alpha seed data tenant-tagged, `pos-bulk-loader` tenant-aware with a target tenant per job, roles loaded into the platform tenant join the template and `reconcileTemplate(tenant)` propagates them; first-administrator activation token (decisions of 2026-09-10); provisioning defaults per module, documentation | backend, durion | 1 - 2 | WS2b, WS3 |
