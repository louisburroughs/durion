...
| WS2b | **Done 2026-09-10** (backend #1926 identity: module adopted, `ext_tenant`, login tenant resolution, `tid`, `X-Tenant-Id`, `/v1/tenants/me`; backend #1927 provisioning: `template_key`, platform role template + `PLATFORM_ADMIN` seed, `tenant.created` handler with first admin and `tenant.provisioned`, `platform:*` off `ADMIN`). Bulk-loaded roles join the template in WS8 | backend | 2.5 - 3 (spent ~1.5) | WS1, WS2a |
