...
| Platform support access (decided 2026-09-10) | **Impersonation token, never a cross-tenant role.** No role reads across tenants; `PLATFORM_ADMIN` gains `platform:tenant:impersonate` and mints a short-lived (15 min, no refresh) token with `tid` = the target tenant, `act` = the operator and a fixed tenant `SUPPORT` role, audited on issue; the gateway and every module stay unchanged (WS2b-4) |
