...
| Global (non-tenant) data | Explicitly whitelisted per module with a `@TenantGlobal` marker and no RLS policy: vehicle reference data (NHTSA, CarAPI), fitment, tax rate tables, the permission catalog, Flyway history, tenant replicas; plus the one RLS exception decided 2026-09-10, the receiver's `emitted_event` hypertable (TimescaleDB excludes row security from compression and continuous aggregates), whose `tenant_id` data column every query names |
