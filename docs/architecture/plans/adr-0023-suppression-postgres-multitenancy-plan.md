...
| First administrator credential (decided 2026-09-10) | **Operator-delivered activation token.** Provisioning leaves the first `ADMIN` credential-expired; a `platform:tenant:provision` caller mints a one-time 72-hour token through a narrow `pos-security-service` endpoint (a sanctioned provisioning binding); unauthenticated `POST /v1/auth/activate {token, password}` sets the password. No mail needed; the same token later drives e-mail reset |
...
| WS4 | *Pilot half done with WS1 (Kafka header, consumer interceptor, outbox `tenant_id` data column on `pos-location`); the envelope field and the producer signature change remain.* Async platform: envelope `tenantId`, Kafka header, consumer `RecordInterceptor`, outbox as a global table with `tenant_id` data, producer signature change (31 sites); per-tenant manifests and replay; `RemoteTenantRegistry` (decided 2026-09-10) | backend | 1 - 2 | WS1 |
