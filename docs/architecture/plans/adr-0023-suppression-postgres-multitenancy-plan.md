...
| WS3 | Per-module retrofit x 27. Schema done 2026-09-09 (Appendix B). Done 2026-09-10: wave 1 `pos-inventory` (backend #1928), wave 2 `pos-accounting` (#1929), **wave 3 `pos-workorder`** (#1930: 45 scoped entities, outbox `tenant_id`, 3 platform-scoped and 2 per-tenant jobs). Next, largest first: `pos-catalog`, then the rest | backend | 8 - 11 (spent ~1.5) | WS1, WS2 |
