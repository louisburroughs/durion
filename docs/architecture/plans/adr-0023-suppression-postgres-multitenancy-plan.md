...
| WS2b | **Part 1 done 2026-09-10** (backend #1926): `pos-security-service` adopted, `ext_tenant` consumer, login tenant resolution (host and form), `tid` on both tokens, refresh scoped to the token's tenant, `X-Tenant-Id`/`X-Tenant-Slug` at the gateway, `/v1/tenants/me`. **Remaining:** role template, provisioning handler, `tenant.provisioned` producer, `ROLE_PLATFORM_ADMIN` | backend | 2.5 - 3 (spent ~1) | WS1, WS2a |
...
3. **WS2a** `pos-tenant`, then **WS2b** identity and gateway; from this point every request in the integration cell
   ...
   only tokens issued before it and unbound internal paths. Provisioning and the platform role template follow.*
