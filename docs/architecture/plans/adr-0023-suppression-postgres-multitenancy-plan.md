...
| WS8 | Seed and bulk load: alpha seed data tenant-tagged, `pos-bulk-loader` with a target tenant per job, platform-tenant roles join the template via `reconcileTemplate(tenant)`, first-administrator activation token and platform impersonation token (decided 2026-09-10); provisioning defaults per module, documentation | backend, durion | 1 - 2 | WS2b, WS3 |
