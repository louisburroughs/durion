...
A backend table is tenant-isolated only if it carries the exact column, policy, index and
constraint set below; anything short of it leaks across tenants in a way row-level security cannot
recover, and this page is that contract plus the checklist for adding a table to it.

Every persisting module's Flyway history was flattened on 2026-09-09 into a single baseline with
the ADR-0062 row-level multitenancy schema folded in. The decision record is
[ADR-0062](../../adr/0062-postgres-row-level-multitenancy.adr.md); the workstreams are in
[the ADR-0023 suppression plan](../plans/adr-0023-suppression-postgres-multitenancy-plan.md).

> **Reading paths in this document.** Bare paths in backticks (`scripts/db/tenancy/flatten.py`,
...
`pos-inquiry` has no Flyway migrations and is outside this contract. It does **not** build its
schema either: `ddl-auto: none` (`pos-inquiry/src/main/resources/application.yml:6`). It is a stub
module with no persisting behaviour of its own, so it has no tables to isolate.
...
**Every persisting module has adopted the library.** The WS3 waves are complete, so the transitional
default is the only thing still standing between the fleet and strict tenancy — not an adoption
backlog. The authoritative list of adopted modules is the `ADOPTED_MODULES` array in
[`pos-archunit`'s `TenancyArchitectureTest`](../../../../durion-positivity-backend/pos-archunit/src/test/java/com/positivity/archunit/TenancyArchitectureTest.java),
which is also what decides whether a module's entities, schedulers and native queries are judged by
the ArchUnit rules. Read it there: a per-wave roll-call in this document drifts the moment a module
is added, and a module missing from `ADOPTED_MODULES` is not merely undocumented — it is
**unenforced**, and its rules pass vacuously.
