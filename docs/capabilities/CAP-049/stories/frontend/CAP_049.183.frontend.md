...
- Assignees: None
- Capability: CAP_049
- Labels: domain:accounting, frontend, user, type:story, status:needs-review, CAP:049
- Created: 2025-12-24T17:04:54Z
- Updated: 2026-01-27T00:27:43Z
- Closed: N/A
...
- Author: louisburroughs
- Created: 2026-01-25T17:29:55Z
- Updated: 2026-01-25T17:29:55Z
...
### Key Findings:
- **Backend Entity:** AccountingEvent (eventId, eventType, payload JSONB, status, journalEntryId, errorMessage)
- **Status Model:** AccountingEventStatus enum (RECEIVED → PROCESSING → PROCESSED/FAILED/SUSPENDED)
- **Retry Policy:** FAILED and SUSPENDED retriable; SCHEMA_VALIDATION_FAILED and DUPLICATE_EVENT not retriable
- **Endpoints:** 5 REST endpoints (list, detail, submit, retry, processing-log)
- **Permissions:** 5 tokens defined (accounting:workcompleted_events:{view,submit,retry,suspend,view_payload})
- **Operator Reason:** Optional retryReason field (500 char max) for audit trail
- **Payload Visibility:** Permission-gated per AD-009 (accounting:workcompleted_events:view_payload)
- **Navigation:** Accounting → Integrations → Event Ingestion

### Documentation:
See [accounting-questions.md](https://github.com/louisburroughs/durion/blob/master/domains/accounting/accounting-questions.md#issue-183-workcompleted-event-ingestion) for complete backend contract specifications, event lifecycle, and acceptance criteria.

### Ready for Implementation:
✅ Event lifecycle documented  
✅ Retry rules specified  
✅ Permission tokens defined  
✅ Payload visibility policy established  
