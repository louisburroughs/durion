...
- **Backend API contract via Moqui proxy** for:
  ...
  - Submitting return-to-stock transaction and receiving `inventoryReturnId` (or equivalent); backend submit contract exists, but frontend proxy shape is still required
...
> **Blocking:** This story cannot be implemented without confirmed Moqui proxy endpoints and payload shapes for “returnable items”, “reason codes”, and “submit return”. Inventory domain docs provide proxy patterns and error schema expectations, but do not define this specific return-to-stock contract.
...
1. **Backend contract (blocking):** What are the exact Moqui proxy endpoints and payload shapes for:
   ...
   Note: the remaining uncertainty is primarily the read-side and proxy shape, not the existence of a return submit command.
2. **Destination semantics (blocking):** For return-to-stock, what is the required destination model?
   - Is destination always the work order’s site `locationId` (immutable), or user-selectable?
