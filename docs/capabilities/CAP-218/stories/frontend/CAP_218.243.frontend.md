...
- Moqui UI flow to:
  ...
  - submit consumption to backend via **Moqui proxy** (no direct Vue → backend) (DECISION-INVENTORY-002)
...
- Backend endpoint(s) to:
  - load “picked items for workorder” (contract TBD; likely WorkExec-owned)
...
> Inventory domain decisions define proxy pattern, error schema expectations, and correlation/auth behavior. The **specific WorkExec “picked items” contract and the Inventory “consume” contract are not provided** in the inventory domain docs; they must be confirmed.
