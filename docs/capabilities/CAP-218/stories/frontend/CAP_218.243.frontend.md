...
- Frontend UI flow to:
  ...
  - submit consumption through a workorder-owned frontend-facing integration/API surface that orchestrates inventory server-to-server; no direct browser calls to internal inventory services (DECISION-INVENTORY-002)
...
- **Systems:** Work Execution / Workorder service (frontend-facing workorder context and same-origin facade), Inventory service (authoritative pick-task and ledger movement owner)
...
- Backend endpoint(s) to:
  - load “picked items for workorder” through a workorder-owned facade that can aggregate workorder and inventory data
  - submit consumption/issue transaction through a workorder-owned facade that invokes inventory server-to-server
- Raw backend evidence already available:
  - `GET /v1/workorders/{workorderId}/detail` in `pos-workorder` returns workorder part lines and includes `quantityIssued` / `quantityConsumed`
  - `GET /v1/inventory/pick-lists?workorderId={workorderId}` and `GET /v1/inventory/pick-lists/{pickListId}/tasks` in `pos-inventory` expose pick-list and pick-task data
  - `POST /v1/inventory/consumption` in `pos-inventory` records picked-item consumption
  - `POST /v1/workorders/{workorderId}/parts/consume` exists in `pos-workorder`, but current implementation is explicitly local workorder part-usage tracking and does not create inventory transactions
...
- PickedItem (workorder-owned facade read model, likely backed by inventory pick lists/tasks plus workorder part context)
- Consumption command payload (submitted to workorder-owned facade; fulfilled via inventory-owned movement command)
...
**PickedItem (read)**
- `pickedItemId` (string, required; facade-owned stable line identifier, expected to map to inventory `pickTaskId`)
...
- Linkage fields (optional, if provided):
  ...
  - `pickListId` / `pickTaskId` (string) when the facade preserves inventory identifiers for submit/audit mapping
...
> Inventory domain decisions define the integration pattern, error schema expectations, and correlation/auth behavior. Backend code now makes the ownership split clearer: inventory owns pick lists, pick tasks, and picked-item consumption; workorder owns the workorder detail read model. For the frontend, the canonical browser contract should still be a workorder-owned facade that orchestrates inventory server-to-server.

### Load/view calls

1. **Get picked items for workorder** (canonical frontend direction resolved; facade path still TBD)
   - Browser-facing route should be workorder-owned, same-origin, and facade/BFF-style:
     - `GET <TBD>/workorders/{workorderId}/picked-items`
   - Why this is the recommended frontend contract:
     - `pos-workorder` already owns workorder detail and exposes workorder part totals through `GET /v1/workorders/{workorderId}/detail`
     - `pos-inventory` already owns pick-list/task data through:
       - `GET /v1/inventory/pick-lists?workorderId={workorderId}`
       - `GET /v1/inventory/pick-lists/{pickListId}/tasks`
     - the browser should not call internal inventory services directly for this workflow
   - Current gap:
     - no existing backend facade yet exposes a single aggregated picked-items read model for the frontend
   - Request:
     - `GET <TBD>/workorders/{workorderId}/picked-items` (placeholder for workorder-owned facade)
...
1. **Inventory-owned movement command** (backend-to-backend reference surface)
   ...
   - Notes:
     ...
     - This is the authoritative inventory movement command the workorder-owned facade should invoke.
...
2. **Existing workorder part-usage command** (not canonical for picked-item fulfillment)
   ...
   - Notes:
     ...
     - Current implementation is explicitly a local workorder usage tracker and does **not** create inventory transactions or check inventory availability.
   - Success response fields:
     ```json
     {
       "id": "uuid",
...
3. **Canonical consume surface** (resolved frontend direction; facade route still TBD)
   - Frontend should call a workorder-owned facade route, not `pos-inventory` directly:
     - `POST <TBD>/workorders/{workorderId}/consume-picked-items`
   - Backend orchestration behind that route should:
     - validate workorder context via workorder-owned logic
     - translate facade line identifiers to inventory `pickTaskId` / `skuId` payloads
     - invoke `POST /v1/inventory/consumption` server-to-server
     - return a normalized frontend response with correlation/reference details
   - The existing `pos-workorder` `/parts/consume` endpoint is not the canonical write surface for this story because it does not decrement inventory ledger state.
   - Errors:
...
**Then** the frontend submits a request to the approved workorder-owned route containing `workorderId = WO-123` and the selected picked-line identifier with quantity 2  
**And** the backend translates that request into the inventory consumption command for the relevant picked task(s)  
...
1. What is the **authoritative route naming and placement** for the workorder-owned facade (exact URL path and how Workorder detail links to it)?
2. What is the exact **picked-items facade read model** for a workorder?
   - Backend evidence now supports a composed model from:
     - workorder detail in `pos-workorder`
     - pick-list/task data in `pos-inventory`
   - Remaining definition needed:
     - stable browser-facing line identifier
     - whether the facade exposes raw `pickListId` / `pickTaskId`
     - whether `qtyConsumed` comes from workorder totals, inventory state, or a normalized merge
3. What permission string(s) gate:
...
4. Quantity precision rules:
   - inventory consume currently documents integer quantities
   - workorder part totals are decimal-capable
   Which rule is canonical for the facade contract, and what precision/step should the UI enforce per line?
5. On success, what identifiers should be displayed to the user?
   - inventory returns `consumptionId` / `pickListId`
   - the workorder-owned facade may choose to normalize or rename these
   Which identifiers are canonical and safe to display in this UI?
6. Should the workorder-owned facade support idempotency for consume retries?
   - `pos-workorder` part usage documents `Idempotency-Key`
   - `pos-inventory` consumption currently does not
   If the facade adds idempotency, what request key/header should the frontend send?
