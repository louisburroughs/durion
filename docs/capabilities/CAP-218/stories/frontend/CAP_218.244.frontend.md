...
### Dependencies
- **Domain ownership resolved:** Mechanic picking is Work Execution-owned. Frontend should target a workorder-facing picking facade that orchestrates inventory updates server-to-server rather than call inventory execution endpoints directly.
- **Backend pick scaffold is still required (blocking):** This story depends on the workorder-facing pick-list / pick-task scaffold identified in `#92` and tracked in `durion-positivity-backend#179`. Current `pos-workorder` endpoints expose parts usage operations (`issue`, `consume`, `return`) but do not expose the pick-task load / scan / confirm / complete contract this screen needs.
- **Backend API/state-model detail is still required (blocking):** This story cannot be fully buildable without confirming:
...
- Integration constraint: UI should call the frontend application's backend facade only; no browser-direct inventory execution calls. (DECISION-INVENTORY-002)
...
> **Blocking**: this story still depends on the backend pick scaffold called out in `#92` and tracked in `durion-positivity-backend#179`. The placeholder endpoints below describe the expected WorkExec/workorder-facing contract, but that contract is not implemented yet in `pos-workorder`.
...
1. **Backend contract (blocking):** Implement and publish the exact workorder-facing endpoints and payloads for:
   - loading pick task/lines
   - resolving a scan
   - confirming a pick line
   - completing the pick task  
   This is the missing workorder pick scaffold identified in `#92` and tracked in `durion-positivity-backend#179`. Provide URL paths, request/response JSON, deterministic error schema, and idempotency/locking mechanism.

2. **Identifier to route by (blocking):** Confirm whether the workorder-facing facade is canonically routed by `workOrderId` or `pickTaskId`. If both are supported, define canonical route + deep-link params.

3. **Scan semantics (blocking):** What can be scanned?
...
4. **Multi-match handling (blocking):** If a scan matches multiple pick lines (same SKU appears multiple times), what is the correct disambiguation key (location, lot, work step)?

5. **Quantity rules (partially resolved, backend contract still needed):** Partial picks are allowed. The remaining contract questions are whether task completion is allowed with remaining quantities and whether any approval path exists for exceptions beyond standard backend rejection of over-picks.

6. **Serial/lot control (blocking):** Are items serialized or lot-controlled in this workflow? If yes, what additional capture is required at pick time?

7. **Permissions resolved:** Use JWT token `authorities` claims for frontend gating. Canonical permission names should follow the manifest-aligned inventory keys for pick-list view/execute, with backend `403` still treated as the source of truth.

8. **Task/line state model (blocking):** What are the canonical statuses for pick tasks and pick lines, and what transitions are allowed?
...
Summary:
- Domain ownership: WorkExecution (`pos-workorder`) owns picking; Inventory updates ledger.
...
Backend & Permissions:
- Frontend should target a workorder-facing backend facade for pick tasks (list/execute); Inventory ledger updates remain server-to-server via `pos-inventory`.
- Permissions: `inventory:pick_list:view`, `inventory:pick_list:execute` (location-scoped per RBAC; surfaced to frontend via JWT `authorities` claims).

Next steps:
- Implement the workorder-facing pick scaffold identified in `#92` and tracked in `durion-positivity-backend#179`, then wire `#244` against that contract.
