...
- Frontend calls the approved frontend-facing integration/API surface to load pick list + tasks (no direct browser calls to internal backend services) (DECISION-INVENTORY-002)
...
3. Backend provides an API to retrieve the pick list and tasks for a work order; inventory contracts now exist, but the canonical frontend retrieval/orchestration path is still blocking.
...
> **Blocking:** Inventory domain rules provided do not define the final frontend-facing pick-list retrieval shape. This story now treats the screen as WorkExec-owned, but still requires explicit endpoint/response mapping to be buildable.

### Load/view calls (required)

1. **Get pick list by work order**
   - Method/Path: **TBD** (must be exposed through the approved frontend-facing integration surface; plain JSON) (DECISION-INVENTORY-002, DECISION-INVENTORY-003)
...
## 16. Open Questions
1. **Frontend-facing endpoint contracts (blocking):** What are the exact frontend-facing endpoints (paths, params) and response shapes to:
   ...
   Inventory-side create/list contracts now exist; WorkExec ownership is resolved, and the remaining question is the final frontend-facing contract shape.
