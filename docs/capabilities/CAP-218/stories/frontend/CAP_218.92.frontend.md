...
- Frontend calls a WorkExec/workorder-owned frontend-facing integration/API surface that orchestrates inventory state APIs server-to-server; no direct browser calls to internal inventory services (DECISION-INVENTORY-002)
...
- **Inventory Domain (backend):** system of record for pick list/task state, ordering primitives, and suggested locations.
- **Work Execution Domain (backend):** system of record for work order context and intended frontend-facing orchestration.
- **Ownership note:** parts picking for workorder execution is Workorder Execution-owned.
...
3. Backend provides inventory-side APIs to retrieve pick lists and pick tasks, but the canonical frontend retrieval/orchestration path should be WorkExec/workorder-owned and is not yet exposed. Backend tracking is now split between `durion-positivity-backend#28` for inventory-side pick-list/task state and `durion-positivity-backend#179` for the workorder-facing facade this frontend story needs.
4. AuthN is working via the existing frontend session; UI follows 401/403 behavior conventions (DECISION-INVENTORY-012).
...
1. **Workorder detail page/route** (existing, WorkExec area): add navigation link/button “Pick List”
2. **Pick list page/route** (new): view pick list header + tasks list
3. **Pick list print view/route** (new): print-optimized view (minimal chrome)
4. **Pick list mobile view** (optional): may be the same page with responsive behavior; do not fork unless necessary
...
- Call the approved frontend-facing integration/API surface to:
...
- All calls must go through the approved frontend-facing integration surface (DECISION-INVENTORY-002).
...
- `PickList` (inventory-owned state, surfaced through workexec/workorder-owned facade)
- `PickTask` (inventory-owned state, surfaced through workexec/workorder-owned facade)
...
> Backend evidence now clarifies the ownership split: inventory already exposes raw pick-list and pick-task state APIs, but the browser should still integrate through a WorkExec/workorder-owned facade. `pos-workorder` currently has no pick-list controller/endpoint, so the missing piece is a frontend-facing facade contract rather than proof that inventory APIs exist.

### Load/view calls (required)

1. **Get pick list by work order**
   - Method/Path: **TBD** (must be exposed through the approved WorkExec/workorder-facing integration surface; plain JSON) (DECISION-INVENTORY-002, DECISION-INVENTORY-003)
   - Inputs: `workOrderId` (path or query)
   - Backend evidence already available:
     - inventory currently exposes `GET /v1/inventory/pick-lists?workorderId={workorderId}`
     - inventory also exposes `GET /v1/inventory/pick-lists/{pickListId}`
     - no equivalent pick-list endpoint currently exists in `pos-workorder`
...
2. **(If tasks not embedded) Get pick tasks for pick list**
   - Method/Path: **TBD** on the frontend-facing facade
   - Inputs: `pickListId`
   - Backend evidence already available:
     - inventory currently exposes `GET /v1/inventory/pick-lists/{pickListId}/tasks`
     - inventory `PickTaskResponse` already includes `pickTaskId`, `pickListId`, `skuId`, `locationId`, `quantityRequired`, `quantityPicked`, `status`, and `sortOrder`
   - Remaining frontend gap:
     - current inventory task response does not itself promise `productDisplayName`, `uom`, or `storageLocationCode`, so the facade may need to normalize or enrich display fields
...
3. **(Optional) Work order summary**
   - Method/Path: **TBD** (WorkExec-owned)
...
- Backend reference surfaces that already exist but should stay behind the facade:
  - `POST /v1/inventory/pick-lists`
  - `POST /v1/inventory/pick-lists/{pickListId}/release`
- Additional backend evidence:
  - inventory tests explicitly document the intended split: `pos-inventory` owns pick list state and `workexec` owns orchestration
  - `PickListGenerationServiceImpl` is still a scaffold placeholder, so automatic generation/orchestration is not yet an implemented backend contract
...
**Then** the system calls the frontend-facing endpoint to load the pick list for `{workOrderId}`  
...
**Then** the system retries the load call
...
## 16. Open Questions
1. **Frontend-facing facade contracts (blocking):** What are the exact WorkExec/workorder-facing endpoints (paths, params) and response shapes to:
   ...
   Inventory-side create/list/task contracts now exist in `durion-positivity-backend#28`; WorkExec ownership is resolved, and `durion-positivity-backend#179` should provide the final frontend-facing contract shape.
...
3. **Authorization model (blocking):** What permission(s)/roles control:
   - viewing pick lists
   - printing pick lists
4. **Print policy (blocking):** Can `Draft` and/or `NeedsReview` pick lists be printed, and if so is a warning/confirm step required?
5. **Display enrichment (blocking):** If inventory task responses only provide `skuId` / `locationId`, which backend layer supplies:
   - product display name
   - UOM
   - storage location code / printable location label
6. **Backend gap acknowledgment:** `pos-workorder` currently has no pick-list endpoint in code. Is the intended fix:
   - add a workorder/workexec facade endpoint, or
   - expose a gateway/BFF route outside `pos-workorder` that still preserves same-origin frontend integration?
7. **Work order context fields:** Should the pick list view display `scheduledStartAt`, `dueAt`, and `workOrderPriority` and, if so, which service supplies them (inventory response vs workexec summary)?
...
Summary:
- Domain: WorkExecution (`pos-workorder`) should own the frontend-facing pick-list facade, while Inventory remains the system of record for raw pick-list/task state.
- Backend trackers: `durion-positivity-backend#28` for inventory pick-list/task state and `durion-positivity-backend#179` for the WorkExec/workorder-facing facade.

Backend & Permissions:
- Frontend-facing WorkExec contract for list/task retrieval; pos-inventory remains relevant for supporting validations.
- Permissions: `inventory:pick_list:view`; `inventory:pick_list:execute`.
