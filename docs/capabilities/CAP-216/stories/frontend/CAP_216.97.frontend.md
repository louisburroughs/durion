...
> Inventory domain decisions define the integration pattern, error schema, and correlation behavior. The current `pos-inventory` OpenAPI now defines the cross-dock command and receiving-session detail endpoint, but WorkExec read-side support is still only partially specified for frontend use.
...
2. **Find candidate workorders in `pos-workorder`** (partially resolved)
   - Documented endpoints:
     - `GET /v1/workorders` (`getAllWorkorders`) returns an array of `WorkorderResponse`
     - `GET /v1/workexec/wip` (`listWip`) returns paginated active WIP workorders filtered by `locationId`
   - Remaining gap:
     - there is no explicit workorder search endpoint in the current OpenAPI
     - the frontend still needs a canonical choice between raw workorder listing and WIP-oriented listing for the picker flow
3. **Get workorder lines / detail** (partially resolved)
   ...
   - Remaining gap:
     - the detail schema does not clearly guarantee `productSku`
     - the picker still lacks an explicit `remainingQty` field and may need frontend derivation from existing quantity fields or a backend contract addendum
...
1. **Receiving context wiring (partially resolved):** Inventory documents `GET /v1/inventory/receiving/sessions/{sessionId}` for session detail and `POST /v1/inventory/receiving/sessions/{sessionId}/lines/{lineId}/cross-dock` for submit. Remaining questions:
   - how the frontend obtains/stores the `sessionId` + `lineId` context from the receiving flow,
   - whether a dedicated single-line load route is needed,
   - how shipment-detail reload should be routed after submit.
2. **Workorder read contract selection in `pos-workorder` (blocking):** Which existing endpoints should this flow standardize on?
   ...
   - which endpoint is canonical for finding candidate workorders,
   - whether line selection should use `parts[]` from `getWorkorderDetail`,
   - whether `productSku` and `remainingQty` need to be added or derived for picker UX.
3. **Permissions (partially resolved):** Recommended location-scoped permissions are `inventory:receiving:create` and `inventory:receiving:execute`. Remaining question:
   - is there a separate canonical permission string for part-mismatch override?
4. **Auto-issue policy exposure (blocking):** How does the backend communicate whether auto-issue is eligible/selected?
   - explicit `autoIssueEligible` flag, `issueMode` returned pre-submit, or submit-time decision only?
5. **Idempotency (risk):** The cross-dock submit endpoint does not currently document idempotency keys/headers. Confirm whether retries are unsafe or whether an undocumented replay-protection mechanism exists.
6. **Notification status (resolved as omitted in current contract):** The documented submit response does not include a warning/status for WorkExec notification publication, so UI feedback should be treated as asynchronous unless the contract is expanded.
