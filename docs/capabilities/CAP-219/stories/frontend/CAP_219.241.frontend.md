...
**I want** to create a Cycle Count Plan by selecting a Location and one or more Zones and a scheduled date that is today or later in the site timezone,  
...
- UI flow to create a new Cycle Count Plan:
  ...
  - Select **one or more Zones** belonging to the selected physical Location
  - Choose **scheduled date** (must not be in the past; `today` is allowed and `past` is evaluated in the site timezone)
  - Optional free-text metadata:
    - `description` should be optional
    - `planName` may also be optional; current `pos-inventory` OpenAPI exposes optional `planName`
...
- **System (frontend + inventory integration layer):** enforces permission-aware UX, validation handling, and persists plans through the approved backend integration surface.
...
- At least one Location exists (Inventory `LocationRef` list is available via the approved frontend integration surface) (DECISION-INVENTORY-002, DECISION-INVENTORY-008).
- Selected Location has one or more Zones (or UI must handle empty zones).
- Backend endpoints/services exist behind the approved frontend integration surface; the UI must not call inventory backend services directly (DECISION-INVENTORY-002) for:
  - Listing Locations (existing: `GET /inventory/locations`) (DECISION-INVENTORY-008)
  - Listing Zones for a Location, where the zones-by-location endpoint identifies inventory zones by physical location (integration path TBD)
  - Creating a Cycle Count Plan (backend contract exists; integration mapping/details still TBD)
  - (Optional) Fetching a Cycle Count Plan by id for detail view (backend contract exists; integration mapping/details still TBD)
...
1. **Plan List route/page**
   - Lists existing cycle count plans from the plan-list endpoint (minimum columns: plan name/description if present, location, scheduled date, status, createdAt)
   - CTA navigates to create screen
2. **Plan Create route/page**
   - Form to create plan
   - Loads locations on entry
   - Loads zones dynamically after location selection
3. **Plan Detail route/page (optional but recommended)**
...
> Integration pattern: frontend pages call the approved same-origin integration/API surface for inventory/counts behavior; no direct browser calls to internal inventory backend services. (DECISION-INVENTORY-002)
...
5. Sets scheduled date (`today` or later in the site timezone)
6. Optionally enters `description` and/or `planName` when the frontend-facing contract exposes them
7. Submits
...
- **On Plan Create load**:
  - Fetch Locations via the approved frontend integration surface
- **On location change**:
  - Clear any previously selected zones
  - Fetch Zones for chosen location
- **On submit**:
  - Validate required fields client-side
  - Call create plan endpoint via the approved integration/API surface
  - On success:
...
- Use the approved frontend integration/API surface for:
...
- Ensure integration errors are captured and rendered in the page/form error surfaces consistent with frontend conventions.
...
- **Scheduled date cannot be in the past**
  - Client-side: if date is earlier than `today` in the site timezone, show inline error and block submit
  - Server-side: display returned error message; do not override with different wording
- **Optional free-text fields**
  - `description` should be optional
  - `planName` may also be optional; the current `pos-inventory` OpenAPI already models `planName` as optional on create
...
> Counts planning endpoints are **not defined** in the provided Inventory domain decisions. Only the approved frontend integration pattern, error schema conventions, and locations endpoint are authoritative here. Therefore, contracts below are **partially specified** and require clarification before implementation.

### Load/view calls

1. **List Locations (authoritative)**
   - Method: `GET`
   - Path: `/inventory/locations` (same-origin integration surface) (DECISION-INVENTORY-008, DECISION-INVENTORY-002)
...
2. **List Zones by Location**
   - Method: `GET`
   - Path: frontend-facing integration route keyed by physical `locationId` (for example `/inventory/locations/{locationId}/zones`) (DECISION-INVENTORY-002)
   - Query/params: must accept `locationId`
   - Semantics: identifies inventory zones by physical location
   - Response: plain JSON list of zones with `zoneId`, `zoneName`
   - Errors: 401/403; 404 if location not found (if backend chooses); 422 if location invalid (if backend chooses)

3. **List Cycle Count Plans (plan-list)**
   - Method: `GET`
   - Path: frontend-facing integration read route for cycle count plans (for example `/inventory/cycle-count-plans`)
   - Semantics: `plan-list` is the list endpoint for cycle count plans
   ...
   - Filters: optional `locationId`, date range

4. **Get Cycle Count Plan Detail**
   - Method: `GET`
   - Path: frontend-facing detail route backed by inventory `getPlan` (for example `/inventory/cycle-count-plans/{planId}`)
...
5. **Create Cycle Count Plan**
   - Method: `POST`
   - Path: frontend-facing integration route backed by inventory `createPlan` (same-origin integration surface) (DECISION-INVENTORY-002)
   - Request:
     ```json
     {
       "locationId": "uuid",
     ...
     ```
   - Field guidance:
     - `planName` is the canonical optional free-text field in the current backend command contract.
     - `description` may be shown by list/detail read models only if the integration layer adds it; it is not part of the v1 backend create payload.
...
- The create action calls the create-plan integration endpoint and on success navigates to detail/list.
- All calls must include `X-Correlation-Id` header; UI surfaces it on error (DECISION-INVENTORY-012).

### Contract Addendum

- `today` is valid for `scheduledDate`.
- `past` must be evaluated in the site timezone, not the browser timezone.
- For v1 create wiring, `planName` is the canonical optional field and should be sent through to `createPlan`.
- If a UX wants both `planName` and `description`, `description` must remain read-model or proxy-owned metadata until the backend command contract adds it.

### Error handling expectations
- Parse deterministic error schema (DECISION-INVENTORY-003):
  - Map field errors to `locationId`, `zoneIds`, `scheduledDate`, and any optional free-text field exposed by contract (`description` and/or `planName`)
- Unknown errors:
...
- If zones list changes after selection and submit returns 404/422:
  - Show error and prompt user to re-select location/zones; keep scheduled date and any optional `description`/`planName` values
...
**And** I set a scheduled date that is today or later in the site timezone  
...
**Given** I am on the New Cycle Count Plan screen  
**When** I enter a scheduled date that is in the past relative to the site timezone  
...
- Client telemetry/events (if present in app conventions):
  ...
  - Do not log optional free-text content (`description` or `planName`) if it may contain sensitive notes; if logged at all, log only presence/length (security-safe default aligned with DECISION-INVENTORY-011).
...
- **i18n/timezone:** Date validation treats `past` relative to the site timezone, and `today` is allowed. Display dates in local format; send in API-required format (TBD).
...
1. **Zones-by-location read model:** zones are scoped by physical `locationId`; the frontend integration layer should expose a zones-by-location route for the create form.
2. **Plan list read model:** `plan-list` is the canonical list surface for cycle count plans; list rows should at minimum expose `planId`, `locationId`, `zoneIds`, `planName`, `scheduledDate`, `status`, and `createdAt`.
3. **Date validation:** `today` is allowed, and `past` is evaluated in the site timezone.
4. **Optional text fields:** `planName` is the canonical optional command field in v1; `description` is optional only if the integration read model exposes it, and it should not be sent to `createPlan` unless the command contract changes.
5. **Navigation default:** create success may navigate to detail when `getPlan` is available through the integration surface; otherwise the safe default is return to the plan list with success confirmation.
...
Backend & Permissions:
- Frontend-facing inventory integration routes map to pos-inventory counts/plan endpoints.
