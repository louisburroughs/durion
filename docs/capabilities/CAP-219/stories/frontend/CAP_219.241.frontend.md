...
2. **List Zones by Location (TBD)**
   - Method: `GET`
   - Path: **TBD** (must be exposed through the approved integration surface; no direct backend calls) (DECISION-INVENTORY-002)
...
3. **List Cycle Count Plans (TBD path, for PlanList screen)**
   - Method: `GET`
   - Path: **TBD**
   - Semantics: plan-list returns a list of cycle count plans
...
5. **Create Cycle Count Plan (TBD)**
   - Method: `POST`
   - Path: **TBD** (same-origin integration surface) (DECISION-INVENTORY-002)
   - Request (conceptual; current backend OpenAPI explicitly includes optional `planName`):
     ```json
     {
       "locationId": "uuid",
       "zoneIds": ["uuid"],
     ...
     ```
   - Field guidance:
     - `description` should be optional if exposed by the frontend-facing contract
     - `planName` may also be optional; `CreateCycleCountPlanRequest` in `pos-inventory/openapi.yaml` currently includes it as an optional property
...
## 16. Open Questions
1. **Counts API contracts (blocking):** What are the exact frontend-facing endpoints (paths), request/response schemas, and deterministic error schema fields for:
   - list zones by location
   - create cycle count plan
   - list plans (for PlanList)
   - get plan by id (for PlanDetail, if desired)
   The remaining gap is the frontend-facing integration/read-side surface, not the existence of create/get plan contracts.
...
3. **Field shape + limits (blocking):** `description` should be optional, and `planName` may also be optional. Confirm the final frontend-facing field set, max lengths, and allowed characters so the frontend can validate without guessing.
4. **Empty zones/items behavior (blocking):** If selected zones contain no inventory items:
   - Should creation be allowed?
   - Should UI show a warning on success?
   - Is there a distinct status or flag returned?
5. **Post-create navigation (blocking):** Should we navigate to a Plan Detail screen (requires GET by `planId`) or back to list only? If detail is required, confirm endpoint and minimum fields.
