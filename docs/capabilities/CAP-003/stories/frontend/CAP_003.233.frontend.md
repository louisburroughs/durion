...
- Assignees: None
- Capability: CAP_003
- Labels: domain:workexec, frontend, user, type:story, status:needs-review, CAP:003
- Created: 2025-12-24T17:45:57Z
- Updated: 2026-01-27T00:21:31Z
- Closed: N/A
...
### Q1: State Model
- After submit: estimate `status` stays **DRAFT**
- Approval record created with `approvalStatus=PENDING`
- Approval outcome drives estimate status → APPROVED/DECLINED

### Q2: Submit Endpoint
- `POST /api/v1/workexec/estimates/{estimateId}:submit-for-approval`
- Headers: `Idempotency-Key` + `If-Match` with `version`
- Body: `{ "approvalMethod": "SIGNATURE" | "CLICK_CONFIRM" | ..., "consentText": "optional" }`
- Response: EstimateDTO with `approvalStatus=PENDING`, `approvalId`, `expiresAt`

### Q3: Validation Checklist
- ≥1 line item required
- Valid taxCode on each item
- Non-negative totals
- Customer contact info (email/phone) when method requires remote approval
- 422 with field-level errors on failure

### Q4: Permissions
- `workexec.estimate.submit` (fail closed)

### Q5: Approval Method
- Request includes `approvalMethod` enum and optional `consentText`
- Location policy may provide defaults

### Q6: Token Sensitivity
- Response includes `approvalId` only
- Customer-facing links gated by separate endpoint/permission

**Reference:** [workexec-questions.md](https://github.com/louisburroughs/durion/blob/master/domains/workexec/workexec-questions.md#issue-233)
