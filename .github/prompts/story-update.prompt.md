...
- For each candidate frontend story:
	...
	- Build a Story Authoring Agent prompt that injects content in this canonical order (follow exactly):
		1. Story Authoring Agent: include the full Story Authoring Agent text loaded verbatim from an `--agent-file` (if supplied) at the very top of the constructed prompt.
		2. Story Writing Prompt: immediately after the Story Authoring Agent, append the full verbatim contents of `durion/.github/prompts/story-frontend-rewrite.prompt.md`. This provides frontend/Moqui-specific rewrite/create guidance. If no file exist, STOP.
		3. Domain-specific agents: next, for backend matches inspect extraction plan metadata or `labels:` blocks for labels beginning with `domain:`. For each unique `domain:<name>` discovered, load `durion/.github/agents/domains/<name>.md` (or `.txt`) and append its full verbatim contents. Include each unique domain agent once, sorted alphabetically. If no files exist, STOP.
		4. Business Rules: after each appended domain-specific agent, if that domain agent contains the literal marker `**Business Rules:** durion/domains/{domain}/.business-rules/` (with `{domain}` substituted), load files under `durion/domains/{domain}/.business-rules/` and append each file's contents (preceded by a short header with the relative path). If no files exist, STOP.
		5. Story Synopsis: provide a one-line summary of the frontend `before.md` (title + short excerpt). Place synopsis after injected guidance so agents see instructions first, then context.
		6. Backend Labels & Context: list the backend search results (file path, one-line excerpt, extraction-plan ID if available, and labels for each match) so the agent has backend metadata and label context.
		7. Matched pattern details: include the matched pattern value and the exact location in `before.md` where it was found.
		8. Final instruction block: an explicit directive — "Review and update the issue: produce an implementation-ready rewrite (`after.md`) following the Story Authoring Agent rules and any appended domain-specific guidance and business rules. Preserve the original `before.md` content in the `Original Story (Unmodified – For Traceability)` section at the end." Ensure the agent output contains the required section headers.
