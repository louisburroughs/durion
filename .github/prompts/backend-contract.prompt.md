...
10. Apply patch after self review:
   - Use the `edit/editFiles` tool to apply changes to `BACKEND_CONTRACT_GUIDE.md`
   - Use `edit/createFile` tool to create the new `CAP-{capability_id}-backend-contract.md` implementation document
   - Use `github/push_files` tool to commit changes with message "docs(capability): update backend contract guide for {capability_id}"
   - Update the JSON summary with `"applied": true` and report the commit hash
