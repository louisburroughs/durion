...
public record PerformanceMetrics(
    ...
    /**
     * Gets CPU utilization (0-100)
     */
    public double getCpuUtilization() {
        // CPU utilization based on concurrent users relative to max (100)
        return Math.min(100.0, (currentConcurrentUsers / 100.0) * 100.0);
    }
    
    /**
     * Gets memory utilization (0-100)
     */
    public double getMemoryUtilization() {
        // Memory utilization based on request load
        // Higher request volume = higher memory usage
        return Math.min(100.0, (totalRequests / 10000.0) * 100.0);
    }
    
    /**
     * Gets concurrent requests count
     */
