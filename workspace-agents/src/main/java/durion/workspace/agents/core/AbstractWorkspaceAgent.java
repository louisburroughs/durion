...
public abstract class AbstractWorkspaceAgent implements WorkspaceAgent {
    ...
    public AgentResponse processRequest(AgentRequest request) throws AgentException {
        ...
        try {
            ...
            try {
                ...
                if (cause instanceof RuntimeException && cause.getCause() instanceof AgentException) {
                    throw (AgentException) cause.getCause();
                }
                throw new AgentException(agentId, AgentException.AgentErrorType.TIMEOUT,
                        "Request processing failed", e);
            } catch (TimeoutException e) {
                throw new AgentException(agentId, AgentException.AgentErrorType.TIMEOUT,
                        "Request processing timed out", e);
            }
            ...
            // Check if the exception is wrapping an AgentException
            Throwable cause = e.getCause();
    ...
    /**
     * Abstract method for actual request processing - to be implemented by
     * subclasses
     */
