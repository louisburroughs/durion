...
public class FullStackIntegrationAgent implements WorkspaceAgent {
    ...
    public core.IntegrationGuidance generateIntegrationGuidance(java.util.List<?> items) {
        if (items == null || items.isEmpty()) {
            return new core.IntegrationGuidance(java.util.List.of());
        }
        Object first = items.get(0);
        java.util.List<String> names;
        if (first instanceof core.TechnologyStack) {
            names = ((java.util.List<core.TechnologyStack>) items).stream().map(Enum::name).toList();
        } else if (first instanceof core.IntegrationPoint) {
            names = ((java.util.List<core.IntegrationPoint>) items).stream().map(Enum::name).toList();
        } else {
            names = ((java.util.List<?>) items).stream().map(Object::toString).toList();
        }
        return new core.IntegrationGuidance(names);
    }
