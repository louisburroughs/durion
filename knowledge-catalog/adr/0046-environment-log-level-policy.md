---
type: ADR
title: 'ADR-0046: Environment Log Level Policy'
description: 'Every pos-* module in durion-positivity-backend sets logging.level.root: INFO in its base application.yml.'
resource: https://github.com/louisburroughs/durion/blob/master/docs/adr/0046-environment-log-level-policy.adr.md
path: durion/docs/adr/0046-environment-log-level-policy.adr.md
tags: [adr]
status: stable
sources: [docs/adr/0046-environment-log-level-policy.adr.md]
generated: {by: 'script:generate-knowledge-catalog.py', at: '2026-09-15T22:15:27-04:00'}
---

[Canonical ADR](https://github.com/louisburroughs/durion/blob/master/docs/adr/0046-environment-log-level-policy.adr.md) — `docs/adr/0046-environment-log-level-policy.adr.md`

**Status:** Accepted since 2026-07-12

**Related:**

* [ADR-0018](../adr/0018-audit-actor-fields-from-security-context.md)
* [ADR-0045](../adr/0045-autonomous-environment-lifecycle-management.md)
